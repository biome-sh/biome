...
started can be controlled via `BIO_SUP_TEST_NETWORK_SIZE`.

```
make up BIO_SUP_TEST_NETWORK_SIZE=5
```
...
bastion service by setting the `BIO_SUP_TEST_NETWORK_LOG_SERVICE` variable to
`bastion`.

```
make logs BIO_SUP_TEST_NETWORK_LOG_SERVICE=bastion
```
...
can specify a different container using the `BIO_SUP_TEST_NETWORK_PEER_NAME`
variable.

```
make console BIO_SUP_TEST_NETWORK_PEER_NAME=rando_2
```
