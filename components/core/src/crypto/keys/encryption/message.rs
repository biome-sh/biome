use crate::{
    crypto::keys::{NamedRevision, encryption::primitives},
    error::{Error, Result},
};
...
impl fmt::Display for AnonymousBox {
    /// Implements Biome's String formatting for sending signed
...
impl FromStr for AnonymousBox {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        let key_pair = lines
            ...
            .ok_or_else(|| {
                Error::CryptoError(
                    "Corrupt payload, can't read key_pair \
                                                    identifier"
                        .to_string(),
                )
            })?
...
impl SignedBox {
    ...
    pub fn encryptor(&self) -> &NamedRevision {
        &self.encryptor
    }

    pub fn decryptor(&self) -> &NamedRevision {
        &self.decryptor
    }
    ...
    pub fn nonce(&self) -> &primitives::Nonce {
        &self.nonce
    }
...
impl FromStr for SignedBox {
    ...
    fn from_str(s: &str) -> Result<Self> {
        ...
        let encryptor = lines
            ...
            .ok_or_else(|| {
                Error::CryptoError(
                    "Corrupt payload, can't read encryptor \
                                                     identifier"
                        .to_string(),
                )
            })?
        ...
        let decryptor = lines
            ...
            .ok_or_else(|| {
                Error::CryptoError(
                    "Corrupt payload, can't read decryptor key \
                                                     name"
                        .to_string(),
                )
            })?
...
mod tests {
    ...
    mod anonymous {
        ...
        #[test]
        fn to_string() {
            let anonymous = AnonymousBox {
                key_pair: "fhloston-paradise-20200813211603".parse().unwrap(),
