...
use std::{
    fs::File,
    io::{self, BufRead, BufReader, BufWriter, prelude::*},
    path::Path,
};
...
impl ArtifactHeader {
    ...
    pub fn signer(&self) -> &NamedRevision {
        &self.signer
    }
    ...
    P: AsRef<Path>,
{
    ...
    let format = if reader.read_line(&mut line)? == 0 {
        Err(Error::CryptoError(
            "Corrupt payload, can't read format \
                                version"
                .to_string(),
        ))
    } else {
    ...
    let named_revision = if reader.read_line(&mut line)? == 0 {
        Err(Error::CryptoError(
            "Corrupt payload, can't read origin \
                                key name"
                .to_string(),
        ))
    } else {
    ...
    if reader.read_line(&mut line)? == 0 {
        Err(Error::CryptoError(
            "Corrupt payload, can't find end of \
                                header"
                .to_string(),
        ))
    } else {
