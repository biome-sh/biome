...
crate::env_config_socketaddr!(
    OutboundIpAddrLookupSocketAddr,
    BIO_OUTBOUND_IP_ADDR_LOOKUP_SOCKET_ADDR,
...
mod tests {
    ...
    fn ip_lookup() {
        ...
        // We would prefer to use the `locked_env_var` macro here but currently, the `biome_core`
        // crate does not depend on `biome_common`. If this becomes an issue, some refactoring
        // will be needed.
