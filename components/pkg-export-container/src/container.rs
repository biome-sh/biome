use crate::{
    build::BuildRoot,
    engine::Engine,
    naming::{ImageIdentifiers, Naming},
    util,
};
...
impl ContainerImage {
    ...
    pub(crate) fn expanded_identifiers(&self) -> &[String] {
    ...
    pub(crate) fn create_report<P: AsRef<Path>>(&self, ui: &mut UI, dst: P) -> Result<()> {
        ...
        util::write_file(
            &report,
            &Handlebars::new()
                .render_template(BUILD_REPORT, &json)
    ...
    fn report_path<P: AsRef<Path>>(dir: P) -> PathBuf {
        dir.as_ref().join(BUILD_REPORT_FILE_NAME)
    }
    ...
    fn create_old_report<P: AsRef<Path>>(ui: &mut UI, dst: P) {
        ...
        ui.status(
            Status::Creating,
            format!(
                "old build report '{}' for backwards compatibility; please favor '{}' \
                           going forward",
                old_report.display(),
        ...
        if let Err(e) = std::fs::copy(&current_report, &old_report) {
            error!(
                "Failed to create '{}' for backwards-compatibility purposes; this may safely \
                    be ignored: {}",
                old_report.display(),
                e
            );
...
impl BuildContext {
    ...
    fn create_dockerfile(&self, ui: &mut UI) -> Result<()> {
        ...
        let dockerfile = &Handlebars::new()
            .render_template(DOCKERFILE, &json)
            .map_err(|err| anyhow!("{}", err))?;
