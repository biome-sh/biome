...
use clap::{self,
           ArgMatches};
use bio::license;
use biome_common::{command::package::install::{InstallHookMode,
                                                 InstallMode,
                                                 InstallSource,
                                                 LocalPackageUsage},
                     ui::{Status,
...
impl TryFrom<&ArgMatches> for BuildSpec {
    ...
    fn try_from(m: &ArgMatches) -> std::result::Result<Self, Self::Error> {
        ...
        Ok(BuildSpec { bio: m.get_one::<String>("HAB_PKG").unwrap().to_string(),
                       bio_launcher: m.get_one::<String>("HAB_LAUNCHER_PKG")
                                      .unwrap()
                                      .to_string(),
                       bio_sup: m.get_one::<String>("HAB_SUP_PKG").unwrap().to_string(),
...
impl BuildRootContext {
    ...
    pub fn svc_users_and_groups(&self) -> Result<(Vec<EtcPasswdEntry>, Vec<EtcGroupEntry>)> {
        ...
        // line.
        let bio_uid = if uid == super::DEFAULT_HAB_UID {
            super::DEFAULT_HAB_UID + 1
        } else {
            super::DEFAULT_HAB_UID
        };
        let bio_gid = if gid == super::DEFAULT_HAB_GID {
