...
_common_prepare() {
  ...
  if [ -z "$BIO_CARGO_TARGET_DIR" ]; then
    # Used by Cargo to use a pristine, isolated directory for all compilation
    export CARGO_TARGET_DIR="$BIO_CACHE_SRC_PATH/$pkg_dirname"
  else
    export CARGO_TARGET_DIR="$BIO_CARGO_TARGET_DIR"
  fi
...
do_prepare() {
  ...
  # dependency on the `bio` crate for a few function calls. None of
