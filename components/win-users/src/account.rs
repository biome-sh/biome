...
use winapi::{
    shared::{
        minwindef::{BOOL, LPDWORD},
        ntdef::LPCWSTR,
        winerror::*,
    },
    um::{
        winbase::LookupAccountSidW,
        winnt::{PSID, PSID_NAME_USE, SID_NAME_USE},
    },
};
...
impl Account {
    ...
    pub fn from_sid(sid: &str) -> Option<Account> {
        ...
        let name = WideCString::from_vec(name)
            .expect("valid name widestring")
            .to_string_lossy();
        let domain = WideCString::from_vec(domain)
            .expect("valid domain widestring")
            .to_string_lossy();
        Some(Account {
            name,
            system_name: None,
...
mod tests {
    ...
    fn real_account_returns_some() {
        assert!(Account::from_name("Administrator").is_some())
    }

    #[test]
    fn bogus_account_returns_none() {
        assert!(Account::from_name("bogus").is_none())
    }
