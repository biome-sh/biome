...
if (Test-Path Env:\BIO_ROOT_PATH) {
    $script:BIO_ROOT_PATH = "$env:BIO_ROOT_PATH"
} else {
    $script:BIO_ROOT_PATH = "\bio"
}
$resolvedRoot = $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($BIO_ROOT_PATH)
# The default path where source artifacts are downloaded, extracted, & compiled
$BIO_CACHE_SRC_PATH = "$resolvedRoot\cache\src"
# The default download root path for package artifacts, used on package
# installation
$BIO_CACHE_ARTIFACT_PATH = "$resolvedRoot\cache\artifacts"
# The default path where cryptographic keys are stored. If the
# `$BIO_CACHE_KEY_PATH` environment variable is set, this value is overridden,
# otherwise it is set to its default.
if (Test-Path Env:\BIO_CACHE_KEY_PATH) {
    $script:BIO_CACHE_KEY_PATH = "$env:BIO_CACHE_KEY_PATH"
} else {
    $script:BIO_CACHE_KEY_PATH = "$resolvedRoot\cache\keys"
}
# Export the key path for other programs and subshells to use
$env:BIO_CACHE_KEY_PATH = "$script:BIO_CACHE_KEY_PATH"
# The root path containing all locally installed packages
$script:BIO_PKG_PATH = "$resolvedRoot\pkgs"
# The first argument to the script is a Plan context directory, containing a
# `plan.ps1` file
$script:PLAN_CONTEXT = "$Context"
# The default Biome Depot from where to download dependencies. If the URL was
# provided as an option use that, if not use any set `BIO_BLDR_URL`
# environment variable, and otherwise use the default provided.
if ($BldrUrl) {
    $script:BIO_BLDR_URL = "$BldrUrl"
} elseif (Test-Path Env:\BIO_BLDR_URL) {
    $script:BIO_BLDR_URL = "$env:BIO_BLDR_URL"
} else {
    $script:BIO_BLDR_URL = "https://bldr.biome.sh"
}
# Export the Builder URL so all other programs and subshells use this same one
$env:BIO_BLDR_URL = "$script:BIO_BLDR_URL"
if (!(Test-Path Env:\BIO_BLDR_CHANNEL)) {
    $env:BIO_BLDR_CHANNEL = "stable"
}
...
#
# Also note that this only really comes into play if BIO_BLDR_CHANNEL
# has been set to something different.
if (!(Test-Path Env:\BIO_FALLBACK_CHANNEL)) {
    $env:BIO_FALLBACK_CHANNEL = "stable"
}
# Use the refresh channel for dependencies in the core origin
if (!(Test-Path Env:\BIO_REFRESH_CHANNEL)) {
    $env:BIO_REFRESH_CHANNEL = "base"
}
...
$script:pkg_svc_path="$BIO_ROOT_PATH\svc\@__pkg_name__@"
...
# command:
#
# * `$_bio_cmd` (bio cli for signing, hashing, and possibly installing)
...
# which means the variables such as `$_bio_cmd` should **not** be used directly
...
function Set-SystemCommands {
    ...
    if (Test-Command bio) {
        $script:_bio_cmd = (Get-Command bio.exe).Source
    } else {
        throw "We require bio to sign artifacts; aborting"
    }
    Write-Debug "Setting _bio_cmd=$_bio_cmd"
...
function Set-BioBin {
    ...
    if ($env:BIO_BIN) { $script:BIO_BIN=$env:BIO_BIN }
    else { $script:BIO_BIN=$_bio_cmd }
    Write-BuildLine "Using BIO_BIN=$BIO_BIN for installs, signing, and hashing"
}
...
function Install-Dependency($dependency, $install_args = $null) {
    if (!$env:NO_INSTALL_DEPS) {
        ...
        $channel = $env:BIO_BLDR_CHANNEL
        $ignoreLocal = ""
        if ($origin -eq "core" -or $origin -eq "biome") {
            if ($origin -eq "core") {
                $channel = $env:BIO_REFRESH_CHANNEL
            }
            if (!$env:BIO_PREFER_LOCAL_BIOME_DEPS) {
                $ignoreLocal="--ignore-local"
            }
        }
        $cmd = "$BIO_BIN pkg install -u $env:BIO_BLDR_URL --channel $channel $dependency $install_args $ignoreLocal"
        $res = Invoke-Expression $cmd | Out-String
        if ($LASTEXITCODE -ne 0 -and ($channel -ne $env:BIO_FALLBACK_CHANNEL)) {
            Write-BuildLine "Trying to install '$dependency' from '$env:BIO_FALLBACK_CHANNEL'"
            $cmd = "$BIO_BIN pkg install -u $env:BIO_BLDR_URL --channel $env:BIO_FALLBACK_CHANNEL $dependency $install_args $ignoreLocal"
            $res = Invoke-Expression $cmd | Out-String
        }
        Write-Host $res
...
# the environment variable BIO_STUDIO_INSTALL_PKGS before building. This helper function resolves
# the given dependency to the identifier installed from BIO_STUDIO_INSTALL_PKGS.
function __resolve_full_ident($dep) {
    if (-not [string]::IsNullOrEmpty($env:BIO_STUDIO_INSTALL_PKGS)) {
...
#
# ```
# Get-DepsFor /bio/pkgs/acme/a/4.2.2/20160113044458
# # /bio/pkgs/acme/dep-b/1.2.3/20160113033619
# # /bio/pkgs/acme/dep-c/5.0.1/20160113033507
# # /bio/pkgs/acme/dep-d/2.0.0/20160113033539
# # /bio/pkgs/acme/dep-e/10.0.1/20160113033453
# # /bio/pkgs/acme/dep-f/4.2.2/20160113033338
# # /bio/pkgs/acme/dep-g/4.2.2/20160113033319
# ```
#
...
#
# ```
# Get-TdepsFor /bio/pkgs/acme/a/4.2.2/20160113044458
# # /bio/pkgs/acme/dep-b/1.2.3/20160113033619
# # /bio/pkgs/acme/dep-c/5.0.1/20160113033507
# # /bio/pkgs/acme/dep-d/2.0.0/20160113033539
# # /bio/pkgs/acme/dep-e/10.0.1/20160113033453
...
function _print_recursive_deps($dependencies, $qualified, $level) {
    ...
    foreach($dep in $dependencies) {
        ...
        # them.
        if (Test-Path "$BIO_PKG_PATH/$dep/DEPS") {
            _print_recursive_deps -Dependencies (Get-Content "$BIO_PKG_PATH/$dep/DEPS") -Qualified $qualified -Level ($level + 1)
        }
    }
}
...
function Assert-DepsAreValid {
    ...
    $tdeps = $pkg_tdeps_resolved | ForEach-Object {
        $_.Substring((Resolve-Path $BIO_PKG_PATH).Path.Length+1).Replace("\", "/")
    }
...
function Resolve-ScaffoldingDependencyList {
    ...
    if($pkg_scaffolding) {
        ...
        if($resolved) {
            ...
            $scaff_build_deps_resolved+=(Resolve-Path "$BIO_PKG_PATH/$resolved").Path
            $sdeps=(@(Get-DepsFor $resolved) + @(Get-BuildDepsFor $resolved))
            foreach($sdep in $sdeps) {
                $scaff_build_deps += $sdep
                $scaff_build_deps_resolved+=(Resolve-Path "$BIO_PKG_PATH/$sdep").Path
            }
        } else {
...
function Set-BuildTdepsResolved {
    ...
    foreach($dep in $pkg_build_deps_resolved) {
        ...
        foreach($tdep in $tdeps) {
            $tdep=(Resolve-Path "$BIO_PKG_PATH/$tdep").Path
...
function Resolve-BuildDependencyList {
    ...
    foreach($dep in $pkg_build_deps) {
        ...
        if($resolved) {
            ...
            $script:pkg_build_deps_resolved+=(Resolve-Path "$BIO_PKG_PATH/$resolved").Path
        } else {
...
function Resolve-RunDependencyList {
    ...
    foreach($dep in $pkg_deps) {
        ...
        if ($resolved) {
            ...
            $script:pkg_deps_resolved+=(Resolve-Path "$BIO_PKG_PATH/$resolved").Path
        } else {
            throw "Resolving '$dep' failed, should this be built first?"
        }
    }
...
function _verify_file($dst, $sha) {
    ...
    $checksum=($(Get-Sha256 "$BIO_CACHE_SRC_PATH/$dst"))
    if ($sha -eq $checksum) {
...
function _download_file($url, $dst, $sha) {
    Push-Location $BIO_CACHE_SRC_PATH
    try {
        ...
        Invoke-WebRequest $url -OutFile $dst -UserAgent "Biome"
...
# `$BIO_CACHE_SRC_PATH\${$pkg_filename}`. If the source already exists in the
...
# Verify that the package we have in `$BIO_CACHE_SRC_PATH\$pkg_filename` has
...
function Invoke-DefaultClean {
    ...
    $src = "$BIO_CACHE_SRC_PATH\$pkg_dirname"
    if (Test-Path "$src") {
...
# Takes the `$BIO_CACHE_SRC_PATH\$pkg_filename` from the download step, and
# unpacks it, as long as the method of extraction can be determined.
#
# This takes place in the `$BIO_CACHE_SRC_PATH` directory.
#
...
function Invoke-DefaultUnpack {
    if($null -ne $pkg_filename) {
        Expand-Archive -Path "$BIO_CACHE_SRC_PATH/$pkg_filename" -DestinationPath "$BIO_CACHE_SRC_PATH/$pkg_dirname"
    }
}
...
function Set-Environment {
    ...
    foreach ($dep_prefix in $pkg_all_deps_resolved) {
        ...
        if (Test-Path (Join-Path $dep_prefix "LIB_DIRS")) {
            ...
            # necessary for packages created using a release of Biome between
        ...
        if (Test-Path (Join-Path $dep_prefix "INCLUDE_DIRS")) {
            ...
            # is necessary for packages created using a release of Biome
    ...
    New-Item "$BIO_CACHE_SRC_PATH\$pkg_dirname" -ItemType Directory -Force |
        Out-Null
...
# `$BIO_CACHE_SRC_PATH\$pkg_dirname`.
...
function Write-Metadata {
    ...
    $pkg_build_deps_resolved | ForEach-Object {
        Resolve-BioPkgPath $_ | Out-File $pkg_prefix\BUILD_DEPS -Encoding ascii -Append
    }
    $pkg_build_tdeps_resolved | ForEach-Object {
        Resolve-BioPkgPath $_ | Out-File $pkg_prefix\BUILD_TDEPS -Encoding ascii -Append
    }
    $pkg_deps_resolved | ForEach-Object {
        Resolve-BioPkgPath $_ | Out-File $pkg_prefix\DEPS -Encoding ascii -Append
    }
    $pkg_tdeps_resolved | ForEach-Object {
        Resolve-BioPkgPath $_ | Out-File $pkg_prefix\TDEPS -Encoding ascii -Append
    }
...
# **Internal** Create the package artifact with `tar`/`bio pkg sign`
...
function Copy-BuildOutput {
    ...
    $_pkg_blake2bsum = $(& $BIO_BIN pkg hash "$pkg_artifact")
...
# `$pkg_target` would have a static value set when the Biome package for this
# program was built.
if ($pkg_target -eq "@@pkg_target@@") {
    if ($env:BUILD_PKG_TARGET) {
        ...
        # bootstrapping the Biome packaging system.
...
#   "$PLAN_CONTEXT\biome\plan.ps1",
#   "$PLAN_CONTEXT\$pkg_target\plan.ps1",
#   "$PLAN_CONTEXT\biome\$pkg_target\plan.ps1"
...
$candidateTargetPaths = @(
    ...
    (Resolve-RootedPath "$PLAN_CONTEXT\biome\$pkg_target\plan.ps1")
)
$candidatePaths = @(
    (Resolve-RootedPath "$PLAN_CONTEXT\plan.ps1"),
    (Resolve-RootedPath "$PLAN_CONTEXT\biome\plan.ps1")
)
...
try {
    ...
    # If the `BIO_ORIGIN` environment variable is set, override the value of
    # `$pkg_origin`.
    if (Test-Path Env:\BIO_ORIGIN) {
        $script:pkg_origin = "$env:BIO_ORIGIN"
    }
    ...
    $script:CACHE_PATH = "$BIO_CACHE_SRC_PATH/$pkg_dirname"
    ...
    $script:pkg_svc_path="$BIO_ROOT_PATH\svc\$pkg_name"
    $script:pkg_svc_data_path="$pkg_svc_path\data"
    $script:pkg_svc_files_path="$pkg_svc_path\files"
    $script:pkg_svc_var_path="$pkg_svc_path\var"
    $script:pkg_svc_config_path="$pkg_svc_path\config"
    ...
    if (-not [string]::IsNullOrEmpty($env:BIO_STUDIO_INSTALL_PKGS)) {
        ...
        $deps = $env:BIO_STUDIO_INSTALL_PKGS -split ";"
        foreach ($dep in $deps) {
            Write-BuildLine "Installing $dep"
            & $BIO_BIN pkg install "$dep"
        }
    }
    ...
    if ($pkg_scaffolding) {
        ...
        $lib="$(Get-BioPackagePath $scaff)/lib/scaffolding.ps1"
    ...
    New-Item "$BIO_CACHE_SRC_PATH" -ItemType Directory -Force | Out-Null
