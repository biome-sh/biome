...
[menu]
  [menu.biome]
    title = "Supervisor Cryptography"
    identifier = "biome/supervisors/Supervisor Cryptography"
...
You can examine the first four lines of a `.bart` file to extract the signature from it, because it is an `xz`-compressed tarball with a metadata header. The `bio pkg header` command will do this for you.

```bash
bio pkg header somefile.bart
```

outputs:

```bash
» Reading package header for somefile.bart

Package        : somefile.bart
Format Version : BART-1
...
The `.bart` file format is designed in this way to allow you to extract both the signature and the payload separately for inspection. To extract only the `xz`-compressed content, bypassing the signature, you could type this:

```bash
tail -n +6 somefile.bart | xzcat | tar x
```
