...
[menu]
  [menu.biome]
    title = "Kubernetes"
    identifier = "biome/containers/kubernetes Kubernetes Development Patterns"
...
All of the Biome Supervisor's state data is stored under `/bio/sup` - we mount this on a persistent volume so it gets re-attached if the pod is ever rescheduled. The data persists!
    ...
    spec:
      ...
      - name: cockroachdb
        ...
        - name: cockroachdb-data
          mountPath: /bio/svc/cockroach/data
  volumeClaimTemplates:
  - metadata:
      name: cockroachdb-data
    spec:
