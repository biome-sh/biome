...
[menu]
  [menu.biome]
    ...
    identifier = "biome/supervisors/updating"
...
This will not update the running Supervisor. It simply downloads the new Supervisor and stores it in your `/bio/pkgs` store. To update the running Supervisor, restart it with:
...
You can configure the Supervisor to automatically update itself when new Supervisor releases become available. This is done by adding the `--auto-update` flag to `bio sup run` or by setting `auto_update` to `true` in `/bio/sup/default/config/sup.toml`. By default, the Supervisor will check for updates every 60 seconds. This can be adjusted by changing the `--auto-update-period` to a different number of seconds.
