...
[menu]
  [menu.biome]
    title = "Supervisor Log Configuration"
    identifier = "biome/supervisors/sup-log-configuration"
...
Place an appropriate YAML configuration file at `/bio/sup/default/config/log.yml` when the Supervisor starts up if you wish to take advantage of this style of configuration. Note that if such a file is present, it will take priority over any `RUST_LOG` environment variable that may also be present.
