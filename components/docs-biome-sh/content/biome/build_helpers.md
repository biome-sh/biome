...
[menu]
  [menu.biome]
    title = "Build Helpers"
    identifier = "biome/reference/build-helpers"
...
pkg\_path\_for()/Get-BioPackagePath
...
For example, to replace all the files in `node_modules/.bin` that have `#!/usr/bin/env` with the coreutils path to `bin/env` (/bio/pkgs/core/coreutils/8.24/20160219013458/bin/env), you must quote the wildcard target as shown below.
...
{{< foundation_tabs_panels tabs-id="bash-powershell-panel2" >}}
  ...
  build_date=$(cat $BIO_CACHE_SRC_PATH/$pkg_filename \
  ...
  function pkg_version {
    ...
    foreach($line in (Get-Content "$BIO_CACHE_SRC_PATH/$pkg_filename")) {
