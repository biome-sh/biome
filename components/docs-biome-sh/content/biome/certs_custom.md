...
[menu]
  [menu.biome]
    title = "Custom Certificates"
    identifier = "biome/reference/certs-custom Custom Certs"
...
Now Biome knows to look for custom certificates in the `~/.bio/cache/ssl` directory, which is `/bio/cache/ssl` when you are running as root.
