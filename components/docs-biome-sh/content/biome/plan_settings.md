...
[menu]
  [menu.biome]
    title = "Plan Settings"
    identifier = "biome/reference/plan-settings"
...
pkg_origin
: The name of the origin for this package. Can contain upper and lowercase letters, numbers, dashes, and underscores. The `BIO_ORIGIN` environment variable overrides the `pkg_origin` Type: string. _Required_.
...
pkg_svc_user
: The user to run the service as. Default: `bio`. On Windows, if the `bio` user does not exist then the service will run under the same account as the Supervisor. _Optional_.
