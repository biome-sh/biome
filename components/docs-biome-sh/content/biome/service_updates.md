...
[menu]
  [menu.biome]
    title = "Single Service Updates"
    identifier = "biome/services/service-updates Individual Configuration Updates"
    parent = "biome/services"
    weight = 40
+++
...
```bash
BIO_PACKAGENAME='{"keyname1":"newvalue1", "tablename1":{"keyname2":"newvalue2"}}'
```

```bash
BIO_MYTUTORIALAPP='{"message":"Biome rocks!"}' bio run <origin>/<packagename>
```
...
```bash
BIO_MYTUTORIALAPP="$(cat my-env-stuff.toml)" bio run
...
```bash
BIO_MYTUTORIALAPP="$(cat my-env-stuff.toml)" bio run <origin>/mytutorialapp
```
...
file that is referenced in [`bio config apply`]({{< relref "biome_cli#bio-config-apply" >}}).
