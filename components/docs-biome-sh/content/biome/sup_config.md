...
[menu]
  [menu.biome]
    title = "Supervisor Configuration File"
    identifier = "biome/supervisors/sup_config How to Rollback"
...
1. The fixed config file location `/bio/sup/default/config/sup.toml` on the Supervisor
...
### Specify an alternate Builder endpoint. If not specified, the value will be taken from the BIO_BLDR_URL environment variable if defined. (default: https://bldr.biome.sh)
