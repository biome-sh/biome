...
[menu]
  [menu.biome]
    title = "Running Biome on Servers (Linux and Windows)"
    identifier = "biome/supervisors/running-biome-servers"
...
First, you must [install Biome]({{< relref "install_biome" >}}) itself on the machine.

Second, many packages default to running as the `bio` user, so you should ensure that both a `bio` user and group exist:
...
A basic systemd unit file for Biome might look like this. This assumes that you have already created the `bio` user and group, as instructed above, and that your `bio` binary is linked to `/bin/bio`.
...
As with Linux, you must first [install Biome]({{< relref "install_biome" >}}) on the machine. Unlike Linux, however, the Windows Supervisor has no requirements for any `bio` user.
