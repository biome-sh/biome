...
1. If you don't already have one, [create an origin]({{< relref "biome/builder_origins" >}}) in your Biome Builder account. This origin will store the Biome package that you create in this tutorial.
...
The [plan file]({{< relref "/biome/plan_writing" >}}) instructs Biome in how to build, deploy, and manage your application.

Edit the plan file to use your origin:

1. Open the `sample-node-app/biome/plan.sh` file with your editor of choice.
1. Update the value of `pkg_origin` with the name of the origin in your Biome Builder account.

Note the `pkg_deps` setting. This lists dependencies that your package requires at runtime. This Node.js application requires the [`core/node` package](https://bldr.biome.sh/#/pkgs/core/node/latest), which is maintained by the Biome Core maintainers. You can see a full list of core packages in the [core-plans](https://github.com/biome-sh/core-plans) repository, or by searching for **core** in Biome Builder.

See the [plan settings]({{< relref "/biome/plan_settings" >}}) documentation for more information about the settings in a plan file.

See the [plan writing]({{< relref "/biome/plan_writing#define-your-dependencies" >}}) documentation for more information defining dependencies.
...
1. If you're using Docker, expose port 8000 so you can preview the application in a browser.
   ...
   ```bash
   export BIO_DOCKER_OPTS="-p 8000:8000"
   ```
...
[Biome Builder]({{< relref "/biome/builder_overview" >}}) acts as the core of Biome's Application Delivery Enterprise hub. The Biome Builder SaaS stores plan files where they can be viewed and accessed by the Biome community and then deployed by the Biome Supervisor.
...
1. Once the plan is connected, select **Latest** towards the top of the page and build the package by selecting **Build latest version** on the right side of the page.
   ...
   {{< figure src="/images/biome/builder-build-latest-version.png" alt="Build latest version button." width=250 >}}

   Select **View the output** and then the **View build job** button ({{< svg file="/static/images/biome/builder-view-build-job-icon.svg" >}}) to watch the service building in Biome Builder.
...
1. Load the service into the Studio using the at-once update strategy.
   ...
   The [at-once update strategy]({{< relref "/biome/service_group_updates#at-once-strategy" >}}) will pull down the package from Biome Builder whenever it detects that a new version has been released.
...
1. When Biome Builder finishes creating a new build of the service, select the **Versions** tab, then select the version of the sample-node-app (1.1.0), and then select **Promote to stable** next to the newest build of the service.
   ...
   See the [Supervisor Log Codes]({{< relref "/biome/sup_log_keys" >}}) documentation for an explanation of the different Supervisor log codes.
...
1. In your local clone of the sample-node-app repository, update the `pkg_version` to `1.2.0` in the `plan.sh` file.
   ...
   You might also try updating the message in `biome/config/config.json` to something like: `Hello friend. This is version {{ pkg.version }} of the Biome Node.js sample app.`
...
1. In [Biome Builder](https://bldr.biome.sh/), select **Build latest version** of your package again, then **View the output**, and select the **View build job** button ({{< svg file="/static/images/biome/builder-view-build-job-icon.svg" >}}).
