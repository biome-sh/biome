...
[menu]
  [menu.biome]
    title = "Building Packages"
    identifier = "biome/packages/pkg-build Build your Package"
...
The `bio-origin` subcommand will place the origin key files, originname-_timestamp_.sig.key (the private key) and originname-_timestamp_.pub files (the public key), in the `$HOME/.bio/cache/keys` directory. If you're creating origin keys in the Studio container, or you are running as root on a Linux machine, your keys will be stored in `/bio/cache/keys`.

Because the private key is used to sign your artifact, it should not be shared freely; however, if anyone wants to download and use your artifact, then they must have your public key (.pub) installed in their local `$HOME/.bio/cache/keys` or `/bio/cache/keys` directory. If the origin's public key is not present, Biome attempts to download it from the Builder endpoint specified by the `--url` argument (https://bldr.biome.sh by default) to `bio pkg install`.
...
1. Set `BIO_ORIGIN` to the name of the origin you intend to use before entering the Studio:

    ```bash
    export BIO_ORIGIN=originname
    ```

    This approach overrides the `BIO_ORIGIN` environment variable and imports your public and private origin keys into the Studio environment. It also overrides any `pkg_origin` values in the packages that you build. This is useful because you can use it to build your own artifact, as well as to build your own artifacts from other packages' source code, for example, `originname/node` or `originname/glibc`.

1. Set `BIO_ORIGIN_KEYS` to the names of your origins. If you're using more than one origin, separate them with commas:

    ```bash
    export BIO_ORIGIN_KEYS=originname-internal,originname-test,originname
    ```
...
1. Create and enter a new Biome Studio. If you have defined an origin and origin key during `bio cli setup` or by explicitly setting the `BIO_ORIGIN` and `BIO_ORIGIN_KEYS` environment variables, then type the following:
...
1. Build the artifact in an unattended fashion, passing the name of the origin key to the command.
    ...
    > Similar to the `bio studio enter` command above, the type of studio where the build runs is determined by your host platform and `bio pkg build` takes the same `-D` flag to force a Docker environment if desired.
...
You can use basic Linux commands like `ls` in this environment. You can also use the `help` command the Biome build system provides in this context to see what other functions can help you debug the plan.
...
Navigating
  ...
  exit-program  End the /bio/pkgs/biome/bio-plan-build/2.0.134/20250606094110/bin/bio-plan-build program.
...
```powershell
[BIO-STUDIO] Biome:\src> Set-PSBreakpoint -Command Invoke-Build
```
...
At C:\src\biome\plan.ps1:26 char:23
+ function Invoke-Build {
+                       ~
[BIO-STUDIO] C:\bio\cache\src\biome-aspnet-sample-0.2.0>>
```
