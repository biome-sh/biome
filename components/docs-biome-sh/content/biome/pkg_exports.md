...
[menu]
  [menu.biome]
    title = "Exporting Packages"
    identifier = "biome/packages/pkg-exports Export Biome Packages"
...
1. Please note that when you run this docker container, you will need to pass the `BIO_LICENSE` environment variable into the container in order to accept the Biome license. If you don't, your container will abort at a license acceptance prompt. One way to do this would be `docker run --env BIO_LICENSE=accept-no-persist IMAGE`. Alternatively, if you use a scheduler to run these docker containers, you should add that environment variable to your scheduler configuration.
...
9. Now, start the Supervisor and load your service package using the `bio` binary, which is included in the tar archive:
    ...
    ```bash
    sudo /bio/bin/bio sup run
    sudo /bio/bin/bio svc load <ORIGIN>/<NAME>
    ```
