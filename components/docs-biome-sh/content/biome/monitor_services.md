...
[menu]
  [menu.biome]
    title = "Monitoring Services"
    identifier = "biome/services/service-monitor"
...
export the `BIO_SUP_GATEWAY_AUTH_TOKEN` environment variable before starting the
...
```bash
$ BIO_SUP_GATEWAY_AUTH_TOKEN="sekret" bio sup run
