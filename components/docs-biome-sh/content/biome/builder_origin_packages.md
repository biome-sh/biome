...
[menu]
  [menu.biome]
    ...
    identifier = "biome/builder/origin-packages"
...
Once you have this token, you can set the `BIO_AUTH_TOKEN` [environment variable]({{< relref "environment_variables" >}}) to this value, so that any commands requiring authentication will use it.
...
Your public and private origin keys are located at `~/.bio/cache/keys` on your host machine and at `/bio/cache/keys` inside the studio environment.
...
> **Note:** When running private packages from Builder, it's necessary to add your [Biome access token]({{< relref "builder_profile#create-a-personal-access-token" >}}) to the machine where you intend to deploy the package, via `export BIO_AUTH_TOKEN=<token>`.
...
```bash
$ /bio/bin/bio svc start core/nginx
```

Note: On a clean server, this will download additional packages to satisfy the Supervisor dependencies. You will also require a `bio` group and `bio` user on the system for most services.

## Building Packages with Multiple Plans

If you have a GitHub repository with multiple components inside, you will most likely also have individual plans for those components that are located inside of component subfolders. By default, Builder will only look for a package plan in either the root of the repository, or in a `biome` subfolder at the root. If it does not find a plan file in those locations, it will not automatically issue builds when it detects file changes in the repository.
...
2. Open it and add an entry for each component package that you want to build.
    ...
    # .bldr.toml
    [bio-launcher]
    plan_path = "components/launcher/biome"
    paths = [
      "components/launcher/*",
