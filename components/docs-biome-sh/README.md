...
The Biome Documentation is deployed on https://docs.biome.sh/ using Hugo
modules.
...
1. Update the Biome repository module in `biome/biome-web-docs`.
...
We also require contributors to include their [DCO signoff](https://github.com/biome/biome/blob/master/CONTRIBUTING.md#developer-certification-of-origin-dco)
...
`Signed-off-by: Julia Child <juliachild@biome.sh>`

See our [blog post](https://blog.biome.sh/introducing-developer-certificate-of-origin/)
...
The Biome documentation team can normally merge pull requests within seven days.
...
### Update the Biome Repository Module In `biome/biome-web-docs`

We use [Hugo modules](https://gohugo.io/hugo-modules/) to build Biome's documentation
from multiple repositories. Expeditor will submit a pull request that updates the documentation
in `biome/biome-web-docs` the next time Biome is promoted to stable.
...
documentation changes as they would appear on docs.biome.sh.

### make serve

Running `make serve` will clone a copy of `biome/biome-web-docs` into `components/docs-biome-sh`.
That copy will be configured to build the Biome documentation from `components/docs-biome-sh`
...
If you have a local copy of biome-web-docs cloned into `components/docs-biome-sh`,
running `make clean_all` will delete the SASS files, node modules, and fonts in
`components/docs-biome-sh/biome-web-docs/themes/docs-new` used to
build the docs site in the cloned copy of biome-web-docs. Hugo will reinstall these
the next time you run `make serve`.

## Creating New Pages

Please keep all of the Biome documentation in the `content/biome` directory.
To add a new Markdown file, run the following command from the `components/docs-biome-sh`
directory:

```
hugo new content/biome/<filename>.md
```
...
See our [Style Guide](https://docs.biome.sh/style_guide/) for more information
about formatting documentation using Markdown and Hugo.

## What Happens Behind the Scenes

The [Biome Documentation](https://docs.biome.sh) site uses [Hugo modules](https://gohugo.io/hugo-modules/)
to pull content from `biome-sh/biome/components/docs-biome-sh`. Every time
`biome-sh/biome` is promoted to stable, Expeditor submits a PR to biome-web-docs to
update the version of the `biome-sh/biome` repository that Hugo uses to build Biome
Biome documentation on the [Biome Documentation](https://docs.biome.sh) site.
This is handled by the Expeditor subscriptions in the `biome/biome-web-docs` repository.
...
and then pushed up to https://packages.biome.sh/files/stable/biome/latest/generated-documentation.tar.gz.
Expeditor will copy those two pages from the tarball into the Biome vendored content in `biome-web-docs`
each time it submits a pull request to update the Biome documentation.

See:
- .expeditor/scripts/release_biome/generate-cli-docs.js
- https://github.com/biome/biome-web-docs/blob/master/.expeditor/update_hugo_modules_project_promoted.sh
- https://github.com/biome-sh/biome/pull/7993
...
Release notes are published from the [biome/biome-web-docs repository](https://github.com/biome/biome-web-docs/blob/main/content/release_notes/biome.md).
...
Send an email to Biome-Docs@progress.com for documentation bugs,
ideas, thoughts, and suggestions. This email address isn't a
support email address. If you need support, contact [Biome Support](https://biome.sh/support/).
...
Submit an issue to [biome-web-docs](https://github.com/biome/biome-web-docs/issues) for
