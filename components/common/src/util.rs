...
pub fn resolve_socket_addr_with_default_port<S: AsRef<str>>(
    ...
    addrs
        ...
        .ok_or_else(|| {
            Error::ListenCtlResolutionError(
                addr.to_string(),
                io::Error::new(
...
mod tests {
    ...
    fn test_socket_addr_with_default_port() {
        assert_eq!(
            resolve_socket_addr_with_default_port("127.0.0.1", 89).unwrap(),
            ("127.0.0.1".to_string(), "127.0.0.1:89".parse().expect(""))
        );
        assert_eq!(
            resolve_socket_addr_with_default_port("localhost", 89).unwrap(),
            ("localhost".to_string(), "127.0.0.1:89".parse().expect(""))
        );
        assert_eq!(
            resolve_socket_addr_with_default_port("1.2.3.4:1500", 89).unwrap(),
