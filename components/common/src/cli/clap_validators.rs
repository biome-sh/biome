...
/// Struct implementing validator for Biome Origin
///
/// Validates with `biome_core::origin::Origin::validate` function.
#[derive(Clone)]
pub struct BioOriginValueParser;

impl clap_v4::builder::TypedValueParser for BioOriginValueParser {
    type Value = String;
    ...
    fn parse_ref(&self,
        ...
        let result = biome_core::origin::Origin::validate(val);
        if result.is_err() {
            let mut err =
                clap_v4::Error::new(clap_v4::error::ErrorKind::ValueValidation).with_cmd(cmd);
            if let Some(arg) = arg {
...
// TODO: This will be used by `bio config` (this implements the functionality of
...
use biome_core::package::ident::{FullyQualifiedPackageIdent,
                                   PackageIdent};
