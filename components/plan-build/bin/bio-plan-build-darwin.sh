...
if [[ -n $BIO_NATIVE_PACKAGE ]]; then
  exec /usr/bin/env bash "${source_dir}/bio-plan-build-darwin-internal.bash" "$@"
fi
...
SANDBOX_PROFILE="${BIO_STUDIO_ROOT}/tmp/PLAN_SANDBOX"
(
...
if declare -f runtime_sandbox >/dev/null; then
  runtime_sandbox >"${BIO_STUDIO_ROOT}/tmp/RUNTIME_SANDBOX"
  ret=$(/usr/bin/sandbox-exec -f "${BIO_STUDIO_ROOT}/tmp/RUNTIME_SANDBOX" /bin/false 2>/dev/null || echo $?)
  if [[ "$ret" == "65" ]]; then
    out=$(/usr/bin/sandbox-exec -f "${BIO_STUDIO_ROOT}/tmp/RUNTIME_SANDBOX" /bin/false 2>&1 >/dev/null || echo "")
...
if declare -f buildtime_sandbox >/dev/null; then
  buildtime_sandbox >"${BIO_STUDIO_ROOT}/tmp/BUILDTIME_SANDBOX"
  ret=$(/usr/bin/sandbox-exec -f "${BIO_STUDIO_ROOT}/tmp/BUILDTIME_SANDBOX" /bin/false 2>/dev/null || echo $?)
  if [[ "$ret" == "65" ]]; then
    out=$(/usr/bin/sandbox-exec -f "${BIO_STUDIO_ROOT}/tmp/BUILDTIME_SANDBOX" /bin/false 2>&1 >/dev/null || echo "")
  ...
  (
    ...
    echo "(import \"${BIO_STUDIO_ROOT}/tmp/BUILDTIME_SANDBOX\")"
  ) >>"$SANDBOX_PROFILE"
fi
...
exec /usr/bin/sandbox-exec \
  ...
  -DSTUDIO_BIO="$BIO_STUDIO_BIO_BIN" \
  -DPLAN_CONTEXT_DIR="$PWD" \
  -DPKG_OUTPUT_PATH="$pkg_output_path" \
  "${source_dir}/bio-plan-build-darwin-internal.bash" "${ORIG_PLAN_CONTEXT}" "${@:2}"
