...
# # Synopsis
#
# `bio-plan-build` handles creating Biome packages from Plan files.
#
# # plan.sh
#
# The heart of `bio-plan-build` is the Plan. This file is a shell
...
# ## Plan variables
#
# `bio-plan-build` sets a few useful variables for you, in addition to the ones
...
# * `$pkg_svc_path`: Where the running service is; `$BIO_ROOT_PATH/svc/$pkg_name`
...
# * `$BIO_CACHE_SRC_PATH`: The path to all the externally downloaded package
#      sources
# * `$BIO_CACHE_ARTIFACT_PATH`: The default download root path for package
#      artifacts, used on package installation
# * `$BIO_CACHE_KEY_PATH`: The path where cryptographic keys are stored
...
#
# `bio-plan-build` allows you to alter its behavior by defining callbacks (or build phases)
...
if ((BASH_VERSINFO[0] < 5)); then
  echo "The biome plan build script requires bash version 5.0 or later to be available in the environment PATH, current bash version is ${BASH_VERSINFO[0]}.${BASH_VERSINFO[1]}"
  return 1
fi
...
: "${BIO_CACHE_KEY_PATH:=$BIO_ROOT_PATH/cache/keys}"
# Export the key path for other programs and subshells to use
export BIO_CACHE_KEY_PATH
...
BIO_PLAN_FILENAME="plan.sh"
# The default Biome Depot from where to download dependencies. If
# `BIO_BLDR_URL` is set, this value is overridden.
: "${BIO_BLDR_URL:=https://bldr.biome.sh}"
...
pkg_svc_path="$BIO_ROOT_PATH/svc/@__pkg_name__@"
...
_ensure_origin_key_present() {
  local cache="$BIO_CACHE_KEY_PATH"
  local keys_found
...
_find_system_commands() {
  ...
  if exists wget; then
    ...
    if [[ "${BIO_NONINTERACTIVE:-}" == "true" ]]; then
  ...
  if exists bio; then
    _bio_cmd=$(command -v bio)
  else
    exit_with "We require bio to sign artifacts; aborting" 1
  fi
  debug "Setting _bio_cmd=$_bio_cmd"
...
_latest_installed_package() {
  local result
  if result="$($BIO_BIN pkg path "$1" 2>/dev/null)"; then
    echo "$result"
    return 0
  else
...
#
# ```
# _get_tdeps_for /bio/pkgs/acme/a/4.2.2/20160113044458
...
#
# ```
# _get_deps_for /bio/pkgs/acme/a/4.2.2/20160113044458
# # /bio/pkgs/acme/dep-b/1.2.3/20160113033619
# # /bio/pkgs/acme/dep-c/5.0.1/20160113033507
# # /bio/pkgs/acme/dep-d/2.0.0/20160113033539
# # /bio/pkgs/acme/dep-e/10.0.1/20160113033453
...
# package dependencies, signing, and hashing files. The `$BIO_BIN` variable
# will either be set or emptied according to the following criteria (first
# match wins):
#
# * If a `$BIO_BIN` environment variable is set, then use this as the absolute
#   path to the binary.
...
_determine_bio_bin() {
  ...
  build_line "Using BIO_BIN=$BIO_BIN for installs, signing, and hashing"
}
...
_resolve_scaffolding_dependencies() {
  ...
  if resolved="$(_resolve_dependency "$pkg_scaffolding")"; then
    ...
    for sdep in "${sdeps[@]}"; do
      ...
      scaff_build_deps_resolved+=("$BIO_PKG_PATH/$sdep")
    done
  else
...
_print_recursive_deps() {
  ...
  while read -r dep; do
    ...
    # them.
    if [[ -f $BIO_PKG_PATH/$dep/DEPS ]]; then
      _print_recursive_deps $((level + 1)) <"$BIO_PKG_PATH"/"$dep"/DEPS
    fi
  done
}
...
_write_pre_build_file() {
  ...
  if [[ $_stat_variant == "bsd" ]]; then
    plan_owner="$($_stat_cmd -f '%Su:%g' "$PLAN_CONTEXT/$BIO_PLAN_FILENAME")"
  else
    plan_owner="$($_stat_cmd -c '%u:%g' "$PLAN_CONTEXT/$BIO_PLAN_FILENAME")"
  fi
...
# `$BIO_CACHE_SRC_PATH/$pkg_filename` has the `$pkg_shasum` we expect.
...
# `$BIO_CACHE_SRC_PATH/$pkg_filename` from the download step and unpack it,
...
_do_copy_templates() {
  if [[ -d "$PLAN_CONTEXT/$1" ]]; then
    if [[ -z "${BIO_CONFIG_EXCLUDE:-}" ]]; then
      # BIO_CONFIG_EXCLUDE not set, use defaults
      config_exclude_exts=("*.sw?" "*~" "*.bak")
    else
      IFS=',' read -r -a config_exclude_exts <<<"$BIO_CONFIG_EXCLUDE"
    fi
    find_exclusions=()
...
# as the `$pkg_svc_user` and `$pkg_svc_group`. These are `bio` by
# default.
#
...
do_default_build_service() {
  ...
  if [[ -f "${PLAN_CONTEXT}/hooks/run" || -f "${PLAN_CONTEXT}/hooks/run.*" ]]; then
    ...
    if [[ -n "${pkg_svc_run}" ]]; then
      # We use chpst to ensure that the script works outside `bio-sup`
...
\`\`\`bash
$(cat "$PLAN_CONTEXT"/$BIO_PLAN_FILENAME)
\`\`\`
EOT
  return 0
}
...
_prepare_build_outputs() {
  ...
  _pkg_blake2bsum=$($BIO_BIN pkg hash "$pkg_artifact" | cut -d " " -f 1)
  if [[ $_stat_variant == "bsd" ]]; then
    plan_owner="$($_stat_cmd -f '%Su:%g' "$PLAN_CONTEXT/$BIO_PLAN_FILENAME")"
  else
    plan_owner="$($_stat_cmd -c '%u:%g' "$PLAN_CONTEXT/$BIO_PLAN_FILENAME")"
...
# your code.
SRC_PATH="${BIO_SRC_PATH:=$PLAN_CONTEXT}"
...
# or `./biome/plan.sh`.  The exception to this is when the $pkg_target
...
candidate_target_paths=(
  "$PLAN_CONTEXT/$pkg_target/$BIO_PLAN_FILENAME"
  "$PLAN_CONTEXT/biome/$pkg_target/$BIO_PLAN_FILENAME"
)
candidate_paths=(
  "$PLAN_CONTEXT/$BIO_PLAN_FILENAME"
  "$PLAN_CONTEXT/biome/$BIO_PLAN_FILENAME"
)
...
if [[ ${#paths[@]} -gt 0 && ${#target_paths[@]} -gt 0 ]]; then
  warn "There is a $BIO_PLAN_FILENAME inside $pkg_target and outside as well. Using the plan in $pkg_target."
...
# Load the Plan
build_line "Loading $PLAN_CONTEXT/$BIO_PLAN_FILENAME"
if source "$PLAN_CONTEXT/$BIO_PLAN_FILENAME"; then
...
if [[ -n "$BIO_OUTPUT_PATH" ]]; then
  pkg_output_path="$BIO_OUTPUT_PATH"
else
...
pkg_svc_path="$BIO_ROOT_PATH/svc/$pkg_name"
pkg_svc_data_path="$pkg_svc_path/data"
pkg_svc_files_path="$pkg_svc_path/files"
pkg_svc_var_path="$pkg_svc_path/var"
pkg_svc_config_path="$pkg_svc_path/config"
...
# TODO: avoid double resolution due to previous resolution in generate-sandbox
_resolve_dependencies
