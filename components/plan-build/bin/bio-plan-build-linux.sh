...
: "${BIO_PREFER_LOCAL_BIOME_DEPS:=false}"
...
_resolve_scaffolding_dependencies() {
  ...
  if [[ $resolved != "" ]]; then
    ...
    scaff_build_deps_resolved+=("$BIO_PKG_PATH/$resolved")
...
_resolve_build_dependencies() {
  ...
  for dep in "${pkg_build_deps[@]}"; do
    ...
    if [[ $resolved != "" ]]; then
      ...
      pkg_build_deps_resolved+=("$BIO_PKG_PATH/$resolved")
    else
...
_resolve_run_dependencies() {
  ...
  for dep in "${pkg_deps[@]}"; do
    ...
    if [[ $resolved != "" ]]; then
      ...
      pkg_deps_resolved+=("$BIO_PKG_PATH/$resolved")
    else
      exit_with "Resolving '$dep' failed, should this be built first?" 1
    fi
  done
