...
do_build() {
	...
	sed \
		-e "s,^BIO_PLAN_BUILD=0\.0\.1\$,BIO_PLAN_BUILD=$pkg_version/$pkg_release," \
...
do_install() {
	...
	install -D components/plan-build/bin/bio-plan-build-darwin-internal.bash "$pkg_prefix"/bin/
