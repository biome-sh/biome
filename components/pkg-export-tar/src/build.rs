...
pub(crate) struct BuildSpec<'a> {
    ...
    /// Whether to exclude the bio bin directory from the final bundle
    pub(crate) no_bio_bin: bool,

    /// Excludes supervisor and launcher packages (`biome/bio-sup` and `biome/bio-launcher`)
...
impl<'a> BuildSpec<'a> {
    ...
    async fn install_base_pkgs(&self, ui: &mut UI, rootfs: &Path) -> Result<BasePkgIdents> {
        ...
        let (sup, launcher) = if self.no_bio_sup {
            (None, None)
        } else {
