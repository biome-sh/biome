...
do_download() {
    ...
    "$tar_binary" -czf "$BIO_CACHE_SRC_PATH"/"$pkg_filename" components/ test-services/ Cargo.toml Cargo.lock NOTICES.txt  || exit
...
# shellcheck disable=2154
do_prepare() {
    export CARGO_HOME="$BIO_CACHE_SRC_PATH/$pkg_dirname/.cargo"
    build_line "Setting CARGO_HOME=$CARGO_HOME"

    export CARGO_TARGET_DIR="$BIO_CACHE_SRC_PATH/$pkg_dirname/target"
...
do_build() {
    ...
    cargo build "$build_type" --target="$rustc_target" --verbose --bin bio-pkg-export-tar
    popd >/dev/null || exit
}
...
# shellcheck disable=2154
do_install() {
    install -v -D "$CARGO_TARGET_DIR"/"$rustc_target"/release/bio-pkg-export-tar \
        "$pkg_prefix"/bin/bio-pkg-export-tar
