{{~ #unless minimal }}
# This file is the heart of your application's biome.
# See Overview Docs at https://docs.biome.sh/plan_contents/

################################################################################
# SETTINGS - https://docs.biome.sh/plan_settings/
################################################################################
...
# REQUIRED (unless overridden by the `BIO_ORIGIN` environment variable)
...
################################################################################
# CALLBACKS - https://docs.biome.sh/build_phase_callbacks/
################################################################################

# When defining your plan, you have the flexibility to override the default
# behavior of Biome in each part of the package building stage through a
...
# downloaded, checksum-verified, and placed in $BIO_CACHE_SRC_PATH/$pkgfilename,
# which resolves to a path like /bio/cache/src/filename.tar.gz. You should
...
# The default implementation removes the BIO_CACHE_SRC_PATH/$pkg_dirname folder
...
# BIO_CACHE_SRC_PATH. The supported archives are: .tar, .tar.bz2, .tar.gz,
...
# place the compiled binaries or libraries in BIO_CACHE_SRC_PATH/$pkg_dirname,
# which resolves to a path like /bio/cache/src/packagename-version/. It uses
...
# custom installation steps, such as copying files from BIO_CACHE_SRC_PATH to
