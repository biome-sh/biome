...
use crate::{
    ...
    hcore::{
        self, ChannelIdent,
        fs::{self, FS_ROOT_PATH, cache_artifact_path},
        package::{PackageIdent, PackageInstall, PackageTarget},
        url::default_bldr_url,
    },
};
use biome_common::error::Error as CommonError;
use retry::delay;
...
pub async fn command_from_min_pkg_with_channel(
    ui: &mut UI,
    command: impl Into<PathBuf>,
    ident: &PackageIdent,
...
/// `bio sup run --channel`) passed with the `channel` parameter of
/// this function. However, if you'd rather use a different channel
/// (particularly if you're developing Biome), you'll need to set the
...
async fn command_from_min_pkg_with_optional_channel(
    ui: &mut UI,
    command: impl Into<PathBuf>,
    ident: &PackageIdent,
...
fn internal_tooling_channel(channel: Option<ChannelIdent>) -> ChannelIdent {
    hcore::env::var(INTERNAL_TOOLING_CHANNEL_ENVVAR)
        ...
        .or(channel)
        .or_else(|| option_env!("BIO_RC_CHANNEL").map(ChannelIdent::from))
        .unwrap_or_default()
}
