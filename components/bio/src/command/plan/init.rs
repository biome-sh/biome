...
use biome_core::{origin::Origin, package::PackageIdent};
...
pub fn start(
    ...
    if cfg!(windows) {
        let rendered_plan = handlebars
            .render_template(PLAN_TEMPLATE_PS1, &data)
            .map_err(|e| Error::HandlebarsRenderError(Box::new(e)))?;
        create_with_template(ui, &format!("{}/plan.ps1", root), &rendered_plan)?;
    } else {
        let rendered_plan = handlebars
            .render_template(PLAN_TEMPLATE_SH, &data)
            .map_err(|e| Error::HandlebarsRenderError(Box::new(e)))?;
        create_with_template(ui, &format!("{}/plan.sh", root), &rendered_plan)?;
    }
    ui.para(
        "`plan.sh` is the foundation of your new biome. It contains metadata, \
    ...
    ui.para(
        "For more information on any of the files: \
         https://docs.biome.sh/reference/plan-syntax/",
    )?;
