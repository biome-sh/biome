...
use crate::{
    AUTH_TOKEN_ENVVAR, BLDR_URL_ENVVAR, ORIGIN_ENVVAR, REFRESH_CHANNEL_ENVVAR, command,
    common::ui::{UI, UIReader, UIWriter},
    error::Result,
};
use biome_common::{cli::CTL_SECRET_ENVVAR, cli_config::CliConfig};
#[cfg(windows)]
use biome_core::fs::{self, FS_ROOT_PATH};
use biome_core::{crypto::keys::KeyCache, env as henv, origin::Origin};
#[cfg(windows)]
use std::env;
#[cfg(windows)]
use std::path::Path;
...
pub fn start(ui: &mut UI, key_cache: &KeyCache) -> Result<()> {
    ...
    ui.para("Welcome to bio cli setup. Let's get started.")?;
    ...
    if ask_default_builder_instance(ui)? {
        ui.para(
            "Enter the url of your builder instance. The default is \
                 https://bldr.biome.sh. The configured endpoint can be overridden any time \
                 with a `BIO_BLDR_URL` environment variable or a --url flag on the cli.",
        )?;
        ...
        ui.para(
            "Alright, maybe another time. You can also set a `BIO_BLDR_URL` environment \
    ...
    ui.para(
        "Every package in Biome belongs to an origin, which indicates the person or \
    ...
    ui.para(
        "Selecting a default origin tells package building operations such as 'bio pkg \
    ...
    ui.para(
        "For more information on origins and how they are used in building packages, please \
             consult the docs at https://docs.biome.sh/create-packages-build/",
    )?;
    if ask_default_origin(ui)? {
        ui.br()?;
        ui.para(
            "Enter the name of your origin. If you plan to publish your packages publicly, \
        ...
        if is_origin_in_cache(&origin, key_cache) {
            ui.para(&format!(
                "You already have an origin key for {} created and installed. \
                              Great work!",
                &origin
            ))?;
        } else {
            ...
            ui.para(
                "For more information on the use of origin keys, please consult the \
                     documentation at https://docs.biome.sh/concepts-keys/#origin-keys",
            )?;
            if ask_create_origin(ui, &origin)? {
                create_origin(ui, &origin, key_cache)?;
            } else {
                ui.para(&format!(
                    "You might want to create an origin key later with: `bio \
                                  origin key generate {}'",
                    &origin
                ))?;
            }
        ...
        ui.para(
            "Alright, maybe another time. You can also set a `BIO_ORIGIN` environment \
                 variable when using the cli.",
        )?;
    }
    ui.heading("Biome Personal Access Token")?;
    ui.para(
        "While you can perform tasks like building and running Biome packages without \
    ...
    ui.para(
        "The Biome Personal Access Token can be generated via the Builder  Profile page \
             (https://bldr.biome.sh/#/profile). Once you have generated your token, you can \
             enter it here.",
    )?;
    ui.para(
        "If you would like to save your token for use by the Biome client, please enter \
             your access token. Otherwise, just enter No.",
    )?;
    ui.para(
        "For more information on using Builder, please read the \
         documentation at https://docs.biome.sh/using-builder/",
    )?;
    if ask_default_auth_token(ui)? {
        ui.br()?;
        ui.para("Enter your Biome Personal Access Token.")?;
        let auth_token = prompt_auth_token(ui)?;
        write_cli_config_auth_token(&auth_token)?;
    } else {
        ui.para(
            "Alright, maybe another time. You can also set a `BIO_AUTH_TOKEN` environment \
                 variable or pass the `--auth` flag to the cli.",
        )?;
    }
    ui.heading("Supervisor Control Gateway Secret")?;
    ui.para(
        "The Supervisor control gateway secret is used to authenticate bio client commands \
             to a Supervisor. When a new Supervisor is created, a unique secret is generated \
             automatically and stored locally in a file: `/bio/sup/default/CTL_SECRET`. When \
    ...
    if ask_default_ctl_secret(ui)? {
        ui.br()?;
        ui.para("Enter your Biome Supervisor control gateway secret.")?;
        let ctl_secret = prompt_ctl_secret(ui)?;
        write_cli_config_ctl_secret(&ctl_secret)?;
    } else {
        ui.para(
            "Alright, maybe another time. You can also set a `BIO_CTL_SECRET` environment \
                 variable when issuing commands to a remote Supervisor.",
        )?;
    }
    ui.heading("Biome Refresh Channel")?;
    ui.para(
        "The Biome refresh channel determines which channel to use for core packages \
    ...
    ui.para(
        "If you would like to save a default refresh channel for use by the Biome client, \
             please enter your preferred channel. Otherwise, just enter No. Defaults to 'base' \
             if nothing is set.",
    )?;
    if ask_default_refresh_channel(ui)? {
        ui.br()?;
        ui.para("Enter your default Biome refresh channel (e.g., 'stable', 'base').")?;
        let refresh_channel = prompt_refresh_channel(ui)?;
        write_cli_config_refresh_channel(&refresh_channel)?;
    } else {
        ui.para(
            "Alright, maybe another time. You can also set a `BIO_REFRESH_CHANNEL` \
    ...
    #[cfg(windows)]
    {
        ...
        if ui.prompt_yes_no("Add binlink directory to PATH?", Some(true))? {
            if !fs::am_i_root() {
                ui.fatal(
                    "You must be in an elevated console running as an administrator in \
                          order to set the machine's PATH. Please open a new console running as \
                          administrator and run setup again to add the Biome binlink \
                          directory to your path.",
                )?;
            } else {
                set_binlink_path(&binlink_path)?;
                ui.para(&format!(
                    "{} has been added to your path. You will need to open a new \
    ...
    ui.para("That's all for now. Thanks for using Biome!")?;
    Ok(())
}
...
fn prompt_origin(ui: &mut UI) -> Result<Origin> {
    ...
    let default_origin_name = match config.origin {
        Some(o) => {
            ui.para(&format!(
                "You already have a default origin set up as `{}', but feel free \
...
fn ask_default_refresh_channel(ui: &mut UI) -> Result<bool> {
    Ok(ui.prompt_yes_no("Set up a default Biome refresh channel?", Some(false))?)
}
...
fn prompt_auth_token(ui: &mut UI) -> Result<String> {
    ...
    Ok(ui.prompt_ask("Biome personal access token", default.as_deref())?)
}
...
fn prompt_refresh_channel(ui: &mut UI) -> Result<String> {
    ...
    let default = match config.refresh_channel {
        Some(o) => {
            ui.para(
                "You already have a default refresh channel set up, but feel free to change \
                     it if you wish.",
            )?;
            Some(o)
        }
    ...
    Ok(ui.prompt_ask("Biome refresh channel", default.as_deref())?)
}
