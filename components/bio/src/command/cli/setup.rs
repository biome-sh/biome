...
pub fn start(ui: &mut UI, key_cache: &KeyCache) -> Result<()> {
    ...
    ui.para("Welcome to bio setup. Let's get started.")?;
    ...
    ui.para("Every package in Biome belongs to an origin, which indicates the person or \
    ...
    ui.para("Selecting a default origin tells package building operations such as 'bio pkg \
    ...
    if ask_default_origin(ui)? {
        ...
        if is_origin_in_cache(&origin, key_cache) {
            ...
            if ask_create_origin(ui, &origin)? {
                ...
                ui.para(&format!("You might want to create an origin key later with: `bio \
                                  origin key generate {}'",
                                 &origin))?;
            }
        }
    ...
    ui.para("The Supervisor control gateway secret is used to authenticate bio client commands \
...
fn ask_default_ctl_secret(ui: &mut UI) -> Result<bool> {
    Ok(ui.prompt_yes_no("Set up a default Biome Supervisor control gateway secret?",
                        Some(false))?)
}
...
fn prompt_ctl_secret(ui: &mut UI) -> Result<String> {
    ...
    Ok(ui.prompt_ask("Biome Supervisor control gateway secret",
                     default.as_deref())?)
}
