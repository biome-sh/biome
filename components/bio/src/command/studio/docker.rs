...
use crate::{
    VERSION,
    command::studio::enter::{ARTIFACT_PATH_ENVVAR, CERT_PATH_ENVVAR, SSL_CERT_FILE_ENVVAR},
    common::ui::{UI, tty},
    error::{Error, Result},
    hcore::{
        crypto::CACHE_KEY_PATH_ENV_VAR,
        env as henv,
        fs::{CACHE_ARTIFACT_PATH, CACHE_KEY_PATH, CACHE_KEY_PATH_POSTFIX, CACHE_SSL_PATH},
        os::process,
        package::target,
        util::docker,
    },
...
const DOCKER_IMAGE_ENVVAR: &str = "BIO_DOCKER_STUDIO_IMAGE";
const DOCKER_OPTS_ENVVAR: &str = "BIO_DOCKER_OPTS";
const DOCKER_SOCKET: &str = "/var/run/docker.sock";
const BIO_STUDIO_SECRET: &str = "BIO_STUDIO_SECRET_";
...
pub fn start_docker_studio(_ui: &mut UI, args: &[OsString]) -> Result<()> {
    ...
    let mut env_vars = vec![
        ...
        String::from("BIO_AUTH_TOKEN"),
        String::from("BIO_BLDR_URL"),
        String::from("BIO_BLDR_CHANNEL"),
        String::from("BIO_NOCOLORING"),
        String::from("BIO_LICENSE"),
        String::from("BIO_ORIGIN"),
        String::from("BIO_ORIGIN_KEYS"),
        String::from("BIO_REFRESH_CHANNEL"),
        String::from("BIO_STUDIO_INSTALL_PKGS"),
        String::from("BIO_STUDIO_BACKLINE_PKG"),
        String::from("BIO_STUDIO_NOPROFILE"),
        String::from("BIO_STUDIO_NOSTUDIORC"),
        String::from("BIO_STUDIO_SUP"),
    ...
    for (key, _) in env::vars() {
        if key.starts_with(BIO_STUDIO_SECRET) {
            env_vars.push(key);
        }
    }
    ...
    if let Some(index) = args.iter().position(|x| *x == to_cull) {
        args.remove(index);
        let refresh_channel_key = format!("{}{}", BIO_STUDIO_SECRET, "BIO_REFRESH_CHANNEL");
    ...
    T: AsRef<str>,
{
    ...
    for var in env_vars {
        ...
        } else if var.as_ref() == "BIO_LICENSE" && license::license_exists() {
            debug!(
                "Bio license already accepted. Setting container env var: \
                    BIO_LICENSE=accept-no-persist"
            );
            cmd_args.push("--env".into());
            cmd_args.push("BIO_LICENSE=accept-no-persist".to_string().into());
        }
    }
...
mod tests {
    ...
    use crate::{
        command::studio::enter::SSL_CERT_FILE_ENVVAR,
        hcore::{fs::CACHE_SSL_PATH, package::target},
    };
    ...
    fn retrieve_supported_image_identifier() {
        assert_eq!(
            image_identifier(None, target::X86_64_DARWIN),
            format!("{}-{}:{}", DOCKER_IMAGE, "x86_64-linux", VERSION)
        );
        assert_eq!(
            image_identifier(None, target::X86_64_LINUX),
            format!("{}-{}:{}", DOCKER_IMAGE, "x86_64-linux", VERSION)
        );
        assert_eq!(
        ...
        assert_eq!(
            image_identifier(Some("ltsc2016"), target::X86_64_WINDOWS),
            format!(
                "{}-{}:{}-{}",
        ...
        assert_eq!(
            image_identifier(Some("ltsc2016"), target::X86_64_LINUX),
            format!(
                "{}-{}:{}-{}",
                DOCKER_WINDOWS_IMAGE, "x86_64-windows", "ltsc2016", VERSION
            )
