use crate::{
    BLDR_URL_ENVVAR, ORIGIN_ENVVAR, REFRESH_CHANNEL_ENVVAR,
    common::ui::{UI, UIWriter},
    error::{Error, Result},
    hcore::{crypto::CACHE_KEY_PATH_ENV_VAR, env as henv, fs},
};
use biome_common::cli_config::CliConfig;
use biome_core::{AUTH_TOKEN_ENVVAR, package::target::PackageTarget};
use log::{debug, warn};
...
pub const STUDIO_HOST_ARCH_ENVVAR: &str = "BIO_STUDIO_SECRET_BIO_STUDIO_HOST_ARCH";
...
//  bio studio.  It must be set outside of studio and passed in through
...
fn cache_ssl_cert_file(cert_file: &str, cert_cache_dir: &Path) -> Result<()> {
    ...
    if cache_file.exists() && cert_path.exists() && is_same_file(&cache_file, cert_path)? {
        return Err(Error::CacheSslCertError(
            "Source and destination certificate are the same \
                                             file"
                .to_string(),
        ));
    }
...
pub async fn start(ui: &mut UI, args: &[OsString]) -> Result<()> {
    ...
    if config.ctl_secret.is_some() {
        ui.warn(
            "Your Supervisor CtlGateway secret is not being copied to the Studio \
                 environment because the Studio's Supervisor is local. If you wish to contact a \
                 remote Supervisor from the Studio, please set the BIO_CTL_SECRET variable with \
                 your secret.",
        )?;
    }
    ...
    fn rerun_with_sudo_if_needed(ui: &mut UI, args: &[OsString], preserve_path: bool) -> Result<()> {
        ...
        match find_command(SUDO_CMD) {
            ...
            None => {
                ...
                ui.warn(
                    "Running Biome Studio requires root or administrator privileges. \
...
mod inner {
    use crate::{
        ...
        error::{Error, Result},
        exec,
        hcore::{crypto::init, fs::find_command, os::process, package::PackageIdent},
    };
    ...
    pub async fn start_windows_studio(ui: &mut UI, args: &[OsString]) -> Result<()> {
        ...
        if let Some(cmd) = find_command(&pwsh_command) {
            // we need to disable the default ctrlc handler for this process (bio)
