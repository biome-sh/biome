...
#[cfg(target_os = "linux")]
const BIO_PLAN_BUILD_SOURCE_FILES: [(&str, &[u8]); 4] = [
    (
        "environment.bash",
...
fn start_native_studio_impl(_ui: &mut UI, args: &[OsString]) -> anyhow::Result<(), anyhow::Error> {
    ...
    fs::create_dir_all(&build_dir).context(
        "Failed to create cache build directory for the \
                                            biome plan build script",
    )?;
    for (source_file_name, source_file_data) in BIO_PLAN_BUILD_SOURCE_FILES {
    ...
    cmd.arg(build_dir.join("bio-plan-build.sh"))
        .arg(args.last().unwrap())
        .env("BIO_NATIVE_PACKAGE", "true");

    if let Some(position) = args.iter().position(|x| x == "-s") {
        cmd.env("BIO_SRC_PATH", &args[position + 1]);
    } else {
