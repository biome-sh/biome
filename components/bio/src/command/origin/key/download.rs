...
use biome_core::{
    crypto::keys::{KeyCache, NamedRevision},
    origin::Origin,
};
...
pub async fn download_public_encryption_key(
    ui: &mut UI,
    api_client: &BuilderAPIClient,
    origin: &Origin,
    token: &str,
    ...
    retry_builder_api!(async {
        ...
        let key_path = api_client
            .fetch_origin_public_encryption_key(origin, token, key_cache.as_ref(), ui.progress())
            .await?;
        ui.status(
            Status::Cached,
            key_path.file_name().unwrap().to_str().unwrap(), /* lol */
        )?;
        Ok::<_, biome_api_client::error::Error>(())
    })
    .await
    .map_err(|e| {
        APIClientError(APIFailure::DownloadLatestKeyFailed(
            API_RETRY_COUNT,
            origin.to_string(),
            Box::new(e),
        ))
...
async fn download_secret_key(
    ...
    retry_builder_api!(async {
        ...
        let key_path = api_client
            .fetch_secret_origin_key(origin, token, key_cache.as_ref(), ui.progress())
            .await?;
        ui.status(
            Status::Cached,
            key_path.file_name().unwrap().to_str().unwrap(), /* lol */
...
async fn download_key(
    ...
    if key_cache.public_signing_key(named_revision).is_ok() {
        ...
        .await
        .map_err(|e| {
            APIClientError(APIFailure::DownloadKeyFailed(
                API_RETRY_COUNT,
