...
use biome_core::crypto::keys::{Key, PublicOriginSigningKey, SecretOriginSigningKey};
...
pub async fn start(
...

    {
        retry_builder_api!(async {
            ...
            match api_client
                .put_origin_key(name, rev, public_keyfile, token, ui.progress())
                .await
            {
        ...
        .await
        .map_err(|e| {
            APIClientError(APIFailure::UploadKeyFailed(
                API_RETRY_COUNT,
    ...
    if let Some(secret_keyfile) = secret_keyfile {
        ...
        retry_builder_api!(async {
            ...
            api_client
                .put_origin_secret_key(name, rev, secret_keyfile, token, ui.progress())
                .await
        })
        .await
        .map_err(|e| {
