use crate::{
    ...
    common::ui::{Status, UI, UIReader, UIWriter},
    error::{Error, Result},
};
use biome_core::origin::{Origin, OriginMemberRole};
...
pub async fn start(
    ...
    ui.begin(format!(
        "Preparing to update member {}'s role to '{}' in {} origin.",
    ...
    match api_client
        .update_member_role(origin.clone(), token, member_account, role)
        .await
    {
        Ok(_) => ui
            .status(Status::Updated, "the member role successfully!".to_string())
            .or(Ok(())),
        Err(err @ api_client::Error::APIError(StatusCode::FORBIDDEN, _)) => {
            ui.fatal("Failed to update the role!")?;
            ui.fatal(format!(
                "This situation could arise, if for example, you are not a member \
        ...
        Err(err @ api_client::Error::APIError(StatusCode::UNPROCESSABLE_ENTITY, _)) => {
            ui.fatal("Failed to update the role!")?;
            ui.fatal(format!(
                "This situation could arise, if for example, role: '{}' was no \
        ...
        Err(err @ api_client::Error::APIError(StatusCode::NOT_FOUND, _)) => {
            ui.fatal("Failed to update the role!")?;
            ui.fatal(
                "This situation could arise, if for example, you passed a invalid member \
                      or origin name.",
            )?;
            Err(Error::APIClient(err))
        }
