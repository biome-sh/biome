...
use biome_core::util::text_render::TabularText;
    ...
    match api_client.list_user_invitations(token).await {
        Ok(resp) => {
            ...
            match resp.as_tabbed() {
                ...
                Err(e) => {
                    ui.fatal(format!(
                        "Failed to format origin invitations under your account! \
                                      {:?}.",
                        e
                    ))?;
                    Err(Error::from(e))
        ...
        Err(e) => {
            ui.fatal(format!(
                "Failed to retrieve origin invitations under your account! {:?}.",
                e
            ))?;
            Err(Error::from(e))
        }
