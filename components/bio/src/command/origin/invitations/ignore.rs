...
pub async fn start(ui: &mut UI, bldr_url: &str, origin: &str, token: &str, invitation_id: u64) -> Result<()> {
    ...
    match api_client.ignore_origin_invitation(origin, token, invitation_id).await {
        Ok(_) => ui
            .status(Status::Ignored, "the invitation successfully!".to_string())
            .or(Ok(())),
        Err(e) => {
            ui.fatal(format!(
                "Failed to ignore invitation {} in origin {}, {:?}",
