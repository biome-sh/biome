...
pub async fn start(
    ...
    match api_client
        .send_origin_invitation(origin, token, invitation_account)
        .await
    {
        Ok(_) => ui
            .status(Status::Sent, "the invitation successfully!".to_string())
            .or(Ok(())),
        ...
        Err(e) => {
            ui.fatal(format!(
                "Failed to send invitation to {} in the {} origin, {:?}",
