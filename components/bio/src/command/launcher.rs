use crate::{
    VERSION,
    command::sup::{SUP_CMD, SUP_CMD_ENVVAR, SUP_PKG_IDENT},
    common::ui::UI,
    error::{Error, Result},
    exec,
    hcore::{crypto::init, env as henv, fs::find_command, os::process, package::PackageIdent},
};
...
const LAUNCH_CMD_ENVVAR: &str = "BIO_LAUNCH_BINARY";
...
pub(crate) async fn start_v4(ui: &mut UI, sup_run: SupRunOptions, args: &[OsString]) -> Result<()> {
    ...
    if henv::var(SUP_CMD_ENVVAR).is_err() {
        ...
        exec::command_from_min_pkg_with_channel(
            ui,
            SUP_CMD,
