...
//!
//! ```bash
//! $ bio pkg bulkupload /path/to/artifact_download_dir \
...
use crate::{
    ...
    command,
    common::ui::{Glyph, Status, UI, UIReader, UIWriter},
    error::{Error, Result},
};
use biome_core::{
    ChannelIdent,
    crypto::{
        PUBLIC_KEY_SUFFIX,
        keys::{Key, KeyCache, PublicOriginSigningKey},
    },
};
use glob::glob_with;
use log::debug;
...
pub async fn start(
    ...
    ui.begin(format!(
        "Preparing to upload artifacts to the '{}' channel on {}",
    ...
    if !origins_to_create.is_empty() {
        if !auto_create_origins {
            ui.warn(String::from(
                "Origins are required for uploading the artifacts. The \
    ...
    P: AsRef<Path>,
{
    ...
    glob_with(&path.as_ref().join(pattern).display().to_string(), options)
...
fn ask_create_origins(ui: &mut UI) -> Result<bool> {
    Ok(ui.prompt_yes_no(
        "Create the above missing origins under your Builder account?",
        Some(true),
    )?)
}
