...
pub async fn start(
    ...
    match api_client.delete_package((ident, target), token).await {
        ...
        Err(err @ api_client::Error::APIError(StatusCode::NOT_FOUND, _)) => {
            ui.fatal(format!(
                "This package does not exist, or alternatively, you may need to \
        ...
        Err(err @ api_client::Error::APIError(StatusCode::UNPROCESSABLE_ENTITY, _)) => {
            ui.fatal(format!(
                "Before you can delete this package artifact, demote it from the \
                              `{}` channel\nand remove any reverse dependencies.",
                default_channel_str
            ))?;
            ui.fatal(format!(
                "Demote the package artifact with the command:\nbio pkg demote {} \
                              {} {}",
                ident, default_channel_str, target
            ))?;
            ui.fatal(format!(
                "Discover any reverse dependencies with the command:\nbio pkg \
