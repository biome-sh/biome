//! Downloads a Biome package from a [depot](../depot).
//!
//! # Examples
//!
//! ```bash
//! $ bio pkg download core/redis
//! ```
//!
//! Will download `core/redis` package and all its transitive dependencies from a custom depot:
//!
//! ```bash
//! $ bio pkg download -u http://depot.co:9633 -t x86-64_linux --download-directory download core/redis/3.0.1
//! ```
//!
...
use crate::{
    api_client::{
        ...
        Package, retry_builder_api,
    },
    common::error::Error as CommonError,
    hcore::{
        ChannelIdent, Error as CoreError,
        crypto::{
            artifact,
            keys::{KeyCache, NamedRevision},
        },
        fs::cache_root_path,
...
use biome_common::ui::{Glyph, Status, UIWriter};
    ...
    U: UIWriter,
{
    ...
    debug!(
        "Using download_path {:?} expanded to {:?}",
    ...
    if package_sets.is_empty() {
        ui.fatal(
            "No package identifers provided. Specify identifiers on the command line, or \
                  via a input file",
        )?;
        return Err(CommonError::MissingCLIInputError(String::from(
...
impl DownloadTask<'_> {
        ...
        T: UIWriter,
    {
        ...
        ui.status(
            Status::Downloading,
            format!(
                "Downloading {} artifacts (and their signing keys)",
                expanded_idents.len()
            ),
        )?;
    ...
    async fn determine_latest_from_ident<T>(
        &self,
        ui: &mut T,
        ...
        T: UIWriter,
    {
        ...
        match self
            .fetch_latest_package_in_channel_for(ident, target, channel, self.token)
            .await
        {
            Ok(latest_package) => {
    ...
    async fn get_downloaded_archive<T>(
        &self,
        ui: &mut T,
        ident: &PackageIdent,
        target: PackageTarget,
        ...
        T: UIWriter,
    {
        ...
        .await
        .map_err(|e| {
            APIClientError(APIFailure::DownloadPackageFailed(
                API_RETRY_COUNT,
                ident.clone(),
                target,
                Box::new(e),
    ...
    async fn fetch_keys_and_verify_artifact<T>(
        &self,
        ui: &mut T,
        ident: &PackageIdent,
        target: PackageTarget,
    ...
    async fn fetch_latest_package_in_channel_for(
        &self,
        ident: &PackageIdent,
        target: PackageTarget,
    ...
    fn path_for_keys(&self) -> PathBuf {
        self.download_path.join("keys")
    }

    fn path_for_artifact(&self) -> PathBuf {
        self.download_path.join("artifacts")
    }
        ...
        T: UIWriter,
    {
        ...
        for dir in &system_paths {
            builder.create(dir).map_err(|_| {
                mk_perm_error(format!(
                    "Can't create directory {:?} needed \
                                                           for download",
                    dir
                ))
            })?
        ...
        for dir in &system_paths {
            ...
            if !metadata.is_dir() {
                return Err(mk_perm_error(format!(
                    "{} isn't a directory, needed for \
                                                  download",
                    dir.display()
                )));
            }
...
fn mk_perm_error(msg: String) -> Error {
    CoreError::PermissionFailed(msg).into()
}
