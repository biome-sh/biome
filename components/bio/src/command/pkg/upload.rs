...
use crate::{
    PRODUCT, VERSION,
    api_client::{self, BuilderAPIClient, Client},
    common::{
...
use biome_core::{
    ChannelIdent,
    crypto::{
        artifact::get_artifact_header,
        keys::{KeyCache, KeyFile},
    },
                ...
                match api_client.check_package((&dep, target), Some(token)).await {
                    ...
                    Err(api_client::Error::APIError(StatusCode::NOT_FOUND, _)) => {
                        ...
                        match retry::retry_future!(
                            delay::Fixed::from(RETRY_WAIT).take(RETRIES),
                            attempt_upload_dep(
                                ui,
                                &api_client,
                                token,
...
async fn upload_into_depot(
    ...
    let package_exists_in_target = match api_client
        .put_package(archive, token, force_upload, ui.progress())
        .await
    {
        Ok(_) => true,
...
async fn promote_to_channel(
    ...
    if !default_channel_for_ident && channel != ChannelIdent::unstable() {
        match api_client
            .create_channel(&ident.hacky_get_origin(), &channel, token)
            .await
        {
            Ok(_) => (),
            Err(api_client::Error::APIError(StatusCode::CONFLICT, _)) => (),
...
async fn upload_public_key(
    ...
    match api_client
        .put_origin_key(name, rev, &path_in_cache, token, ui.progress())
        .await
    {
        Ok(()) => {
            ui.begin(format!(
