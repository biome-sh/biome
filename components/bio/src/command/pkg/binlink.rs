...
impl Binlink {
    pub fn new(target: &Path, link: &Path) -> Result<Self> {
        Ok(Self {
            link: Self::binstub_path(target, link)?,
    ...
    fn stub_template(&self, env: BTreeMap<String, String>) -> String {
        ...
        format!(
            include_str!("../../../static/template_binstub.bat"),
...
pub fn start(
    ...
    match Binlink::from_file(&binlink.link) {
        Ok(link) => {
            if force && link.target != src {
                ...
                ui.warn(format!(
                    "Skipping binlink because {} already exists at {}. Use --force \
                                 to overwrite",
                    binary,
                    link.link.display(),
                ))?;
    ...
    if cfg!(target_os = "windows") && !is_dest_on_path(&dst_path) {
        ui.warn(format!(
            "Binlink destination '{}' is not on the PATH. Consider setting it \
                         manually or running 'bio cli setup' to add it to the machine PATH.",
            dst_path.display(),
        ))?;
    }
...
mod tests {
    use super::{Binlink, binlink_all_in_pkg, start};
    use crate::{
        common::ui::UI,
        hcore::{
            self,
        ...
        assert!(
            fs::read_to_string(rootfs_bin_dir.join(magicate_link))
                ...
                .contains(&format!(
                    ...
                    env::join_paths(hcore::fs::windows_system_paths())
                        .unwrap()
                        .to_string_lossy(),
                    "%PATH%"
                ))
