...
use biome_common::{package_graph::PackageGraph,
                     ...
                     ui::{Status,
                          UIWriter}};
use biome_core::{error as herror,
                   fs::{self as hfs,
                        FS_ROOT_PATH},
                ...
                for p in &deps {
                    match graph.count_rdeps(p) {
                        None => {
                            ...
                            // condition in Step 2 with another bio uninstall. We can
...
async fn supervisor_services() -> Result<Vec<PackageIdent>> {
    ...
    while let Some(message_result) = response.next().await {
        ...
        match reply.message_id() {
            "ServiceStatus" => {
                let m = reply.parse::<biome_sup_protocol::types::ServiceStatus>()
                             .map_err(SrvClientError::Decode)?;
                out.push(m.ident.into());
            }
            "NetOk" => (),
            "NetErr" => {
                let err = reply.parse::<biome_sup_protocol::net::NetErr>()
                               .map_err(SrvClientError::Decode)?;
