...
use biome_common::{
    ...
    templating::hooks::{PackageMaintenanceHookExt, UninstallHook},
    ui::{Status, UIWriter},
};
use biome_core::{
    error as herror,
    fs::{self as hfs, FS_ROOT_PATH},
    os::process::Pid,
    package::{Identifiable, PackageIdent, PackageInstall, list::temp_package_directory},
};
...
#[allow(clippy::too_many_arguments)]
pub async fn uninstall_all_but_latest<U>(
    ui: &mut U,
    ident: impl AsRef<PackageIdent>,
    ...
    for ident in idents {
        ...
        match graph.count_rdeps(ident) {
            None => {
                ...
                // Step 2 with another bio uninstall. We can continue as what we
        ...
        match scope {
            ...
            Scope::PackageAndDependencies => {
                ...
                for p in &deps {
                    ...
                    let external_rdeps_count = rdeps
                        .iter()
...
/// Check if we have a launcher/supervisor running out of this biome root.
    ...
    U: UIWriter,
{
    ...
    fs::read_to_string(&pid_file_path)
        ...
        .map(|pid| {
            let is_alive = biome_core::os::process::is_alive(pid);
            if is_alive {
                let _ = ui.status(Status::Found, "running supervisor process");
            } else {
                let _ = ui.warn(
                    "Found stale PID file (process not \
                                                               running)",
                );
            }
            is_alive
    ...
    U: UIWriter,
{
    ...
    let msg = biome_sup_protocol::ctl::SvcStatus::default();
    ...
    while let Some(message_result) = response.next().await {
        ...
        match reply.message_id() {
            "ServiceStatus" => {
                let m = reply
                    .parse::<biome_sup_protocol::types::ServiceStatus>()
    ...
    T: UIWriter,
{
    ...
    if installed.len() > 1 {
        ui.status(
            Status::Skipping,
            format!(
                "execution of uninstall hook for {}. {} packages are installed for {}",
                ident,
                installed.len(),
                unqualified_ident
            ),
        ...
        Some(real_install_base) => {
            ...
            for p in real_install_base.ancestors() {
                ...
                match p.read_dir() {
                    ...
                    Err(e) => return Err(Error::BiomeCore(herror::Error::IO(e))),
                }
            }
            Ok(true)
        }
