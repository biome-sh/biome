...
use crate::{
    ...
    error::{Error, Result},
    exec,
    hcore::{crypto, env as henv, fs::find_command, os::process, package::PackageIdent},
};
...
/// 2. `export_pkg_ident_envvar` is checked for an explicit biome package.
/// 3. The biome package is implicitly determined using the `export_cmd` name and the current
    ...
    match henv::var(export_cmd_envvar) {
        Ok(command) => {
            ...
            if let Some(command) = find_command(&command) {
                debug!(
                    "Using export command {:?} with args `{:?}` specified with envvar `{}`",
