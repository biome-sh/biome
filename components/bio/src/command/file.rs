...
use biome_sup_protocol::{self as sup_proto, butterfly::MAX_FILE_PUT_SIZE_BYTES, net::ErrCode};
...
pub(crate) async fn sub_file_put<U>(
    service_group: &str,
    version: u64,
    file_path: &PathBuf,
    ...
    U: UIWriter,
{
    ...
    let mut msg = sup_proto::ctl::SvcFilePut {
    ...
    match (grp.org(), user_opt) {
        (Some(_org), Some(username)) => {
            ...
            msg.content = Some(
                user_key
                    .encrypt_for_service(&buf, &service_key)?
                    .to_string()
                    .into_bytes(),
            );
