...
use biome_common::{cli_config, error::DEFAULT_ERROR_EXIT_CODE};
            ...
            Error::DockerImageNotFound(ref e) => {
                format!(
                    "The Docker image {} was not found in the docker registry.\nYou can \
                         specify your own Docker image using the BIO_DOCKER_STUDIO_IMAGE \
            ...
            Error::DockerNetworkDown(ref e) => {
                format!(
                    "The Docker image {} is unreachable due to a network error.\nThe image \
                         must be reachable to ensure the versions of bio inside and outside the \
                         studio match.\nYou can specify your own Docker image using the \
                         BIO_DOCKER_STUDIO_IMAGE environment variable.",
                    e
                )
            }
            Error::EnvJoinPathsError(ref err) => format!("{}", err),
            Error::ErrorPerIdent(ref e) => e
                .iter()
...
impl From<cli_config::Error> for Error {
    fn from(err: cli_config::Error) -> Self {
        Error::CliConfig(err)
    }
}
...
impl From<HashMap<PackageIdent, Error>> for Error {
    fn from(errors: HashMap<PackageIdent, Error>) -> Self {
        Error::ErrorPerIdent(errors)
    }
}
