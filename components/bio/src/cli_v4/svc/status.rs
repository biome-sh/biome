...
use std::{convert::Into, io::Write, str::FromStr};
...
use biome_sup_protocol::{
    codec::SrvMessage,
    net::NetErr,
    types::{DesiredState, ProcessState, ServiceStatus},
};
...
impl StatusCommand {
    pub(crate) async fn do_command(&self) -> BioResult<()> {
        let msg = biome_sup_protocol::ctl::SvcStatus {
            ident: self.pkg_ident.clone().map(Into::into),
        };
