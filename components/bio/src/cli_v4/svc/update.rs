...
use biome_sup_protocol::{
    ctl,
    types::{BindingMode, Topology, UpdateCondition, UpdateStrategy},
};

use crate::{
    cli_v4::utils::{BldrUrl, RemoteSup, resolve_channel_for_pkg},
...
impl TryFrom<UpdateCommand> for ctl::SvcUpdate {
    ...
    fn try_from(u: UpdateCommand) -> BioResult<Self> {
        let msg = ctl::SvcUpdate {
            bldr_channel: Some(resolve_channel_for_pkg(&u.channel).to_string()),
