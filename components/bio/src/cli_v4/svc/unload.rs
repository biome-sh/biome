...
/// Unload a service loaded by the Biome Supervisor. If the service is running, it will be stopped
/// first.
...
impl UnloadCommand {
    pub(super) async fn do_command(self) -> BioResult<()> {
        let msg = biome_sup_protocol::ctl::SvcUnload {
            ident: Some(self.pkg_ident.into()),
