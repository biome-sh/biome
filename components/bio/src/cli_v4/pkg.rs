// Implementation of `bio pkg` command
...
pub(super) enum PkgCommand {
    ...
    /// Builds a plan using Biome Studio
    Build(build::PkgBuildOptions),

    /// Bulk uploads Biome artifacts to builder depot from a local directory
    ...
    /// Returns Biome Artifact dependencies, by default the direct dependencies
    /// of the package
