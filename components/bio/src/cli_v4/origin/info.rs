// Implementation of `bio origin info`
...
use biome_core::{
    origin::Origin,
    util::text_render::{PortableText, TabularText},
};
use clap::Parser;
...
impl OriginInfoOptions {
    pub(super) async fn do_info(&self, ui: &mut UI) -> BioResult<()> {
        // Resolve the token
        let token = self
            .auth_token
            .from_cli_or_config()
