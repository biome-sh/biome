// Implementation of `bio origin transfer`
...
use crate::{
    ...
    error::{Error, Result as BioResult},
};
...
impl OriginTransferOptions {
    pub(super) async fn do_transfer(&self, ui: &mut UI) -> BioResult<()> {
        ...
        match api_client
            .transfer_origin_ownership(self.origin.as_ref(), &token, &self.new_owner_account)
            .await
        {
            Ok(_) => ui
                .status(Status::Transferred, "ownership successfully!".to_string())
                .or(Ok(())),
            Err(err @ api_client::Error::APIError(StatusCode::FORBIDDEN, _)) => {
                ui.fatal("Failed to transfer origin ownership!")?;
                ui.fatal(
                    "Either you are not the current owner or the new owner is not yet a \
                          member.",
                )?;
            ...
            Err(err @ api_client::Error::APIError(StatusCode::UNPROCESSABLE_ENTITY, _)) => {
                ui.fatal("Failed to transfer origin ownership!")?;
                ui.fatal(
                    "This situation could arise if, for example, you attempted to transfer \
            ...
            Err(e) => {
                ui.fatal(format!(
                    "Failed to transfer origin {} ownership to {}, {:?}",
