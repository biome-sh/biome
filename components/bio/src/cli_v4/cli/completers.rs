use crate::{cli_v4::Bio, error::Result as BioResult};
...
impl CliCompletersOptions {
    pub(crate) fn do_completers(&self, _feature_flags: FeatureFlag) -> BioResult<()> {
        let mut cmd = Bio::command();

        generate(self.shell, &mut cmd, "bio", &mut io::stdout());
