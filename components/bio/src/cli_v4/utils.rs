...
use biome_common::{
    ...
    types::{GossipListenAddr, ListenCtlAddr, ResolvedListenCtlAddr},
};

use biome_core::{
    AUTH_TOKEN_ENVVAR, ChannelIdent,
...
#[derive(GenConfig, PartialEq, Debug, Clone, Parser, Serialize, Deserialize)]
...
impl Default for CacheKeyPath {
    fn default() -> Self {
        Self {
            cache_key_path: CACHE_KEY_PATH.clone(),
        }
    }
}

impl From<PathBuf> for CacheKeyPath {
    fn from(cache_key_path: PathBuf) -> Self {
        Self { cache_key_path }
    }
}

impl From<&CacheKeyPath> for PathBuf {
    fn from(cache_key_path: &CacheKeyPath) -> PathBuf {
        cache_key_path.cache_key_path.clone()
    }
}
...
pub(crate) struct BldrUrl {
    ...
    /// `BIO_BLDR_URL` environment variable or from the config file if specified.
...
pub(crate) struct AuthToken {
    ...
    /// Authentication token for Builder. Uses value from the `BIO_AUTH_TOKEN` env variable if set
...
impl AuthToken {
    ...
    pub(crate) fn from_cli_or_config(&self) -> BioResult<String> {
        if let Some(auth_token) = &self.auth_token {
            Ok(auth_token.clone())
        } else {
            match hcore_env::var(AUTH_TOKEN_ENVVAR) {
                Ok(v) => Ok(v),
                Err(_) => CliConfig::load()?.auth_token.ok_or_else(|| {
                    BioError::ArgumentError(
                        "No auth token \
                                                                              specified: please \
                                                                              pass `-z/--auth`, \
                                                                              set BIO_AUTH_TOKEN, \
        ...
        match std::env::var(AUTH_TOKEN_ENVVAR) {
            ...
            Err(_) => {
                ...
                cfg.auth_token.clone().ok_or_else(|| {
                    Error::ArgumentError(
                        "No auth token specified: please \
                                                                pass `-z/--auth`, set \
                                                                BIO_AUTH_TOKEN, or add auth_token \
                                                                to ~/.bio/etc/cli.toml"
                            .into(),
                    )
                })
...
#[derive(GenConfig, Debug, Clone, Parser, Serialize, Deserialize)]
...
impl RemoteSup {
    pub fn inner(&self) -> &ResolvedListenCtlAddr {
        &self.remote_sup
    }
}
...
impl TryFrom<String> for SocketAddrProxy {
    ...
    // fn try_from(value: String) -> BioResult<Self> {
...
impl From<&SocketAddrProxy> for SocketAddr {
    fn from(s: &SocketAddrProxy) -> Self {
        s.0
    }
}
...
impl FromStr for SocketAddrProxy {
    //     type Err = BioError;
    //
    // fn from_str(s: &str) -> Result<SocketAddrProxy, BioError> {
    // let (_, addr) = biome_common::util::resolve_socket_addr_with_default_port(
    // s,
    // GossipListenAddr::DEFAULT_PORT,
    // )?;
...
impl FromStr for SubjectAlternativeName {
    //  type Err = BioError;
    //
    // fn from_str(s: &str) -> BioResult<Self> { Ok(SubjectAlternativeName(s.to_string())) }
    type Err = Error;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(SubjectAlternativeName(s.to_string()))
    }
}
...
biome_core::impl_try_from_string_and_into_string!(SubjectAlternativeName);

fn health_check_interval_default() -> u64 {
    30
}

#[derive(GenConfig, PartialEq, Debug, Clone, Parser, Deserialize, Serialize)]
...
pub struct SharedLoad {
    ...
    /// the BIO_BLDR_URL environment variable if defined. (default: https://bldr.biome.sh)
    ...
    topology: Option<biome_sup_protocol::types::Topology>,

    /// The update strategy
    #[arg(long = "strategy", short = 's', default_value = "none", value_enum)]
    #[serde(default)]
    strategy: biome_sup_protocol::types::UpdateStrategy,
    ...
    #[serde(default)]
    binding_mode: biome_sup_protocol::types::BindingMode,
...
impl Default for SharedLoad {
    fn default() -> Self {
        Self {
            ...
            topology: None,
            strategy: biome_sup_protocol::types::UpdateStrategy::None,
            update_condition: UpdateCondition::Latest,
            bind: vec![],
            binding_mode: biome_sup_protocol::types::BindingMode::Strict,
...
// falling back to BIO_ORIGIN envvar or the `cli.toml` config if none was supplied.
pub(crate) fn origin_param_or_env(opt: &Option<CoreOrigin>) -> Result<CoreOrigin, Error> {
    if let Some(o) = opt {
        ...
        // Fallback to BIO_ORIGIN env var
        ...
        cfg.origin.ok_or_else(|| {
            Error::ArgumentError(
                "No origin specified; please set --origin, BIO_ORIGIN, \
...
impl From<PkgIdentStringySerde> for String {
    fn from(pkg_ident: PkgIdentStringySerde) -> Self {
        pkg_ident.to_string()
    }
}
...
impl PkgIdent {
    pub(crate) fn pkg_ident(self) -> PackageIdent {
        self.pkg_ident.0
    }
}
...
impl FromStr for PkgIdent {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(Self {
            pkg_ident: PkgIdentStringySerde(s.parse()?),
        })
    }
}

pub fn shared_load_cli_to_ctl(
    ident: PackageIdent,
    shared_load: SharedLoad,
    force: bool,
) -> BioResult<biome_sup_protocol::ctl::SvcLoad> {
    #[cfg(target_os = "windows")]
    use biome_core::crypto::dpapi;
    use biome_sup_protocol::{
        ctl::{ServiceBindList, SvcLoad},
        types::{HealthCheckInterval, ServiceBind},
    };
    ...
    Ok(SvcLoad {
        bldr_channel: Some(resolve_channel_for_pkg(&shared_load.channel).to_string()),
        ident: Some(ident.into()),
        binds,
        binding_mode: Some(shared_load.binding_mode as i32),
        bldr_url: Some(biome_core::url::bldr_url(shared_load.bldr_url)),
        config_from,
        force: Some(force),
        ...
        health_check_interval: Some(HealthCheckInterval {
            seconds: shared_load.health_check_interval,
        }),
...
#[cfg(not(target_os = "macos"))]
pub(crate) async fn process_sup_request(
    remote_sup: &ResolvedListenCtlAddr,
    ...
    while let Some(message_result) = response.next().await {
        ...
        match reply.message_id() {
            ...
            "NetErr" => {
                let m = reply
                    .parse::<biome_sup_protocol::net::NetErr>()
...
mod tests {
    mod auth_token {
        ...
        biome_core::locked_env_var!(BIO_AUTH_TOKEN, locked_auth_token);
    ...
    mod bldr_url {
        ...
        use crate::cli_v4::utils::{BldrUrl, DEFAULT_BLDR_URL};

        use clap_v4 as clap;

        use clap::Parser;

        biome_core::locked_env_var!(BIO_BLDR_URL, locked_bldr_url);
        ...
        fn env_cli_passed_value() {
            let test_bldr_url_val = "https://test.bldr.biome.sh/";
            let cli_bldr_url_val = "https://cli.bldr.biome.sh/";
            let env_var = locked_bldr_url();
    ...
    mod refresh_channel_tests {
        ...
        biome_core::locked_env_var!(BIO_REFRESH_CHANNEL, locked_refresh_channel);
