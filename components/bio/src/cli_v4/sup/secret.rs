// Implementation of `bio sup secret`
/// Commands relating to a Biome Supervisor's Control Gateway secret
use clap_v4 as clap;

use clap::Parser;

use crate::{cli_v4::utils::SubjectAlternativeName, error::Result as BioResult};
use biome_core::fs::BIO_CTL_KEYS_CACHE;
use std::path::PathBuf;

#[cfg(not(target_os = "macos"))]
use biome_common::ui::{self, UIWriter};
...
pub enum SupSecretOptions {
    ...
    GenerateTls {
        ...
        #[arg(name = "path", default_value = BIO_CTL_KEYS_CACHE)]
        path: PathBuf,
    },
}
...
impl SupSecretOptions {
    #[cfg(not(target_os = "macos"))]
    pub(super) async fn execute(&self) -> BioResult<()> {
        match self {
            ...
            SupSecretOptions::GenerateTls {
                subject_alternative_name,
                path,
            } => Ok(ctl_gateway_tls::generate_self_signed_certificate_and_key(
