...
use crate::{
    cli_v4::utils::{CacheKeyPath, DurationProxy, SharedLoad, SocketAddrProxy, is_default},
    error::{Error as BioError, Result as BioResult},
};

use biome_core::{
    fs::BIO_CTL_KEYS_CACHE,
...
use rants::{Address as NatsAddress, error::Error as RantsError};
...
use biome_core::{env::Config, util as core_util};
...
impl FromStr for EventStreamAddress {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(EventStreamAddress(s.parse()?))
    }
}

impl From<EventStreamAddress> for NatsAddress {
    fn from(address: EventStreamAddress) -> Self {
        address.0
    }
}

#[derive(GenConfig, Debug, Clone, Args, Serialize, Deserialize)]
#[command(
...
pub struct SupRunOptions {
    ...
    #[arg(long = "ctl-server-certificate", default_missing_value = BIO_CTL_KEYS_CACHE, num_args = 0..)]
    pub ctl_server_certificate: Option<CertificateChainCli>,

    /// The control gateway server's private key.
    #[arg(long = "ctl-server-key", default_missing_value = BIO_CTL_KEYS_CACHE, num_args = 0..)]
    ...
    #[arg(long = "ctl-client-ca-certificate", default_missing_value = BIO_CTL_KEYS_CACHE, num_args = 0..)]
    ...
    #[arg(long = "keep-latest-packages", env = "BIO_KEEP_LATEST_PACKAGES")]
...
impl SupRunOptions {
    #[cfg(not(target_os = "macos"))]
    pub(crate) async fn do_run(&self, ui: &mut UI) -> BioResult<()> {
        // Skip "bio" and "sup" so we pass only the subcommand + its args.
    ...
    #[cfg(target_os = "macos")]
    pub(crate) async fn do_run(&self, _ui: &mut UI) -> BioResult<()> {
        Ok(())
    }
}
...
impl SupRunOptions {
    ...
    /// the default config file "/bio/sup/default/config/sup.toml" exists. If that is missing too,
    /// just return the passed 'other' as it is after cleaning the 'config-files' value if any.
    pub fn maybe_merge_from_config_files(mut other: Self) -> BioResult<Self> {
        let config_files = if other.config_files.is_empty() {
            let default_config_path = Path::new("/bio/sup/default/config/sup.toml");
            if default_config_path.exists() && default_config_path.is_file() {
                vec!["/bio/sup/default/config/sup.toml"]
                    .into_iter()
        ...
        for config_file in config_files.into_iter().rev() {
            if is_toml_file(config_file.as_os_str().to_str().unwrap()) {
                ...
                return Err(BioError::ArgumentError(format!(
                    "'{}' is not a valid \
                                                            toml config file.",
                    config_file.display()
                )));
