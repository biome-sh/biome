use crate::{
    cli_v4::utils::{CacheKeyPath, RemoteSup},
    command::config::sub_svc_set,
    error::Result as BioResult,
};
use biome_common::{cli::clap_validators::FileExistsOrStdinValueParser, ui::UI};
use clap::Parser;
use clap_v4 as clap;
...
pub(crate) struct ConfigApplyOptions {
    ...
    /// Supervisor control address (overrides BIO_SUP_CTL_ADDR)
    #[command(flatten)]
