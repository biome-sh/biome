// Implementation of `bio pkg export` command
...
impl PkgExportCommand {
    pub(super) async fn do_export(&self, ui: &mut UI) -> BioResult<()> {
        match self {
            #[cfg(any(target_os = "linux", target_os = "windows"))]
            ...
            PkgExportCommand::Docker(opts) => {
                ui.warn("'bio pkg export docker' is now a deprecated alias for 'bio pkg export \
