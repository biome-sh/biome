// Implementation of `bio pkg build`
use clap_v4 as clap;
...
use biome_core::{crypto, crypto::keys::KeyCache, origin::Origin};
...
pub(crate) struct PkgBuildOptions {
    ...
    #[arg(name = "BIO_ORIGIN_KEYS", short = 'k', long = "keys", action = ArgAction::Append)]
    bio_origin_keys: Vec<Origin>,

    /// Sets the Studio root (default: /bio/studios/<DIR_NAME>)
    #[arg(name = "BIO_STUDIO_ROOT", short = 'r', long = "root")]
    ...
    // TODO : Same as above
    /// A directory containing a plan file or a `biome/` directory which contains the plan
    /// file
    ...
    /// Channel used to retrieve plan dependencies for Biome supported origins.
    /// The value is determined in the following precedence order:
    /// (1) CLI argument (`--refresh-channel` or -f).
    /// (2) `BIO_REFRESH_CHANNEL` environment variable.
...
impl PkgBuildOptions {
    ...
    pub(super) async fn do_build(&self, ui: &mut UI, feature_flags: FeatureFlag) -> BioResult<()> {
        ...
        if self.native_package {
            ...
            if self.reuse || self.docker {
                return Err(BioError::ArgumentError(String::from(
                    "--native-package \
                                                                 conflicts with \
                                                                 --reuse and \
                                                                 --docker",
    ...
    fn should_build_native_package(&self, _: FeatureFlag) -> BioResult<bool> {
        Ok(false)
    }
}
