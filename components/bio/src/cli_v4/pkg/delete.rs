// Implementation of `bio pkg delete` command
...
use biome_common::{
    cli::{PACKAGE_TARGET_ENVVAR, clap_validators::BioPkgIdentValueParser},
    ui::UI,
};

use biome_core::package::{PackageIdent, PackageTarget, target};
...
impl PkgDeleteOptions {
    pub(super) async fn do_delete(&self, ui: &mut UI) -> BioResult<()> {
        ...
        delete::start(ui, &self.bldr_url.to_string(), (&self.pkg_ident, target), &auth_token).await
    }
}
