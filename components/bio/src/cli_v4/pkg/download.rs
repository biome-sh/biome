// Implementation of `bio pkg download` command
...
use biome_common::{
    Error as BiomeCommonError,
    cli::{
        PACKAGE_TARGET_ENVVAR,
        clap_validators::{BioPkgIdentValueParser, TomlOrPkgIdentFileValueParser},
        file_into_idents, is_toml_file,
    },
    ui::UI,
};
...
use biome_core::{
    ChannelIdent,
    env::Config,
    package::{PackageIdent, PackageTarget, target},
};

use crate::{
    PRODUCT, VERSION,
    cli_v4::utils::{AuthToken, BldrUrl},
    command::pkg::{
        download,
...
pub(crate) struct PkgDownloadOptions {
    ...
    #[arg(name = "CHANNEL",
          short = 'c',
          long = "channel",
          env = biome_core::ChannelIdent::ENVVAR)]
    ...
    #[arg(name = "PKG_IDENT", num_args = 1.., value_parser = BioPkgIdentValueParser::simple())]
...
impl PkgDownloadOptions {
    pub(super) async fn do_download(&self, ui: &mut UI) -> BioResult<()> {
        ...
        download::start(
            ...
            auth_token.as_deref(),
            self.verify,
            self.ignore_missing_seed,
        )
        .await
    }

    fn idents_from_file_matches(&self, target: PackageTarget) -> BioResult<Vec<PackageSet>> {
            ...
            for f in &self.pkg_ident_file {
                if is_toml_file(f) {
                    ...
                    let toml_data: PackageSetFile =
                        toml::from_str(&file_data).map_err(BiomeCommonError::TomlParser)?;
                    sources.append(&mut toml_data.to_package_sets()?);
                } else {
                    let idents_from_file = file_into_idents(f)?;
                    match self.channel {
                        Some(ref channel) => sources.push(PackageSet {
                            idents: idents_from_file,
                            channel: channel.clone(),
                            target,
                        }),
                        None => {
                            sources.push(PackageSet {
                                idents: idents_from_file,
                                channel: ChannelIdent::default(),
                                target,
                            });
...
#[cfg(test)]
mod tests {
    use super::{PackageTarget, Parser, PkgDownloadOptions};
