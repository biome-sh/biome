// Implementation of `bio pkg upload` command
...
use biome_common::{cli::clap_validators::FileExistsValueParser, ui::UI};

use biome_core::{ChannelIdent, crypto::keys::KeyCache};

use crate::{
    cli_v4::utils::{AuthToken, BldrUrl, CacheKeyPath},
    command::pkg::upload,
...
impl PkgUploadOptions {
    pub(crate) async fn do_upload(&self, ui: &mut UI) -> BioResult<()> {
