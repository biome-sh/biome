...
//! constitute license acceptance. Setting BIO_LICENSE to "accept" will
//! accept the license and persist the file and setting BIO_LICENSE to
...
const LICENSE_ACCEPT_ENVVAR: &str = "BIO_LICENSE";
...
pub fn check_for_license_acceptance_and_prompt(ui: &mut UI) -> Result<()> {
    match check_for_license_acceptance()? {
        ...
        LicenseAcceptance::NotYetAccepted => {
            ...
            ui.info("View the license at https://biome.sh/end-user-license-agreement")?;
            ui.br()?;
            ...
            if ui.prompt_yes_no("Do you accept the 1 product license?", Some(false))? {
                ...
                ui.br()?;
                ui.info(
                    "If you do not accept this license you will not be able to use Biome \
                         products.",
                )?;
                ui.br()?;
...
pub fn accept_license(ui: &mut UI) -> Result<()> {
    ...
    ui.status(
        ui::Status::Custom(ui::Glyph::CheckMark, String::from("")),
...
fn license_path(root_path: &Path) -> PathBuf {
    root_path.join("accepted-licenses")
}

fn license_file(license_path: &Path) -> PathBuf {
    license_path.join("biome")
}
