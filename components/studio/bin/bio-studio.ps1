...
COMMON OPTIONS:
    ...
    -o <BIO_STUDIO_ROOT>  Sets a Studio root (default: /bio/studios/<DIR_NAME>)
...
ENVIRONMENT VARIABLES:
    ...
    BIO_STUDIO_NOPROFILE        Disables sourcing a \`.studio_profile.ps1' in \`studio enter'
...
function Write-BioInfo($Message) {
    if($quiet) { return }
...
function New-Studio {
    ...
    Write-BioInfo "Creating Studio at $BIO_STUDIO_ROOT"

    if(!(Test-Path $BIO_STUDIO_ROOT)) {
        mkdir $BIO_STUDIO_ROOT | Out-Null
    }

    Set-Location $BIO_STUDIO_ROOT
    ...
    if(!$env:NO_ARTIFACT_PATH) {
        $cachePath = Join-Path $BIO_STUDIO_ROOT "bio/cache"
        ...
        try {
            if(!(Test-Path artifacts)) {
                ...
                New-Item -Name artifacts -ItemType Junction -target "$env:SYSTEMDRIVE/bio/cache/artifacts" | Out-Null
            }
        } finally {
            Pop-Location
        }
    ...
    $pathArray = @(
        ...
        "$env:WINDIR\system32",
        "$env:WINDIR",
        (Join-Path $BIO_STUDIO_ROOT "bio\bin")
    )

    $env:PATH = [String]::Join(";", $pathArray)
    $env:PSModulePath = "$PSScriptRoot\powershell\Modules"

    if($env:BIO_ORIGIN_KEYS) {
        $secret_keys = @()
        $public_keys = @()
        $env:BIO_ORIGIN_KEYS.Split(" ") | ForEach-Object {
            ...
            if($LASTEXITCODE -eq 0) {
                ...
                Write-Host "$(Resolve-Path ~/.bio/cache/keys) (if running in a non-elevated console) or c:\bio\cache\keys"
                ...
                Write-Host "you run 'bio cli setup'."
                Write-Host ""
                Write-Host "Note that if you run 'bio cli setup' in an elevated console as an"
                ...
                Write-Host "created during 'bio cli setup' in a non-elevated console is only"
        ...
        $env:FS_ROOT=$BIO_STUDIO_ROOT
        $env:BIO_CACHE_KEY_PATH = Join-Path $env:FS_ROOT "bio\cache\keys"
        $public_keys | ForEach-Object { $_ | & bio origin key import }
        $secret_keys | ForEach-Object { $_ | & bio origin key import }
    } else {
        Write-Warning "No secret keys imported! Did you mean to set `$env:BIO_ORIGIN?"
        Write-Host "To specify a BIO_ORIGIN, either set the BIO_ORIGIN environment"
        Write-Host "variable to your origin name or run 'bio cli setup' and specify a"
        Write-Host "default origin."
        Write-Host ""
        Write-Host "Note that if you ran 'bio cli setup' in an elevated console as an"
        ...
        Write-Host "specified during 'bio cli setup' in a non-elevated console is only"
        Write-Host "accesible to a non-elevated Studio."
        $env:FS_ROOT=$BIO_STUDIO_ROOT
        $env:BIO_CACHE_KEY_PATH = Join-Path $env:FS_ROOT "bio\cache\keys"
    }

    if (!(Test-Path $env:BIO_CACHE_KEY_PATH)) {
        mkdir $env:BIO_CACHE_KEY_PATH | Out-Null
    }

    $env:BIO_CACHE_SSL_PATH = Join-Path $env:FS_ROOT "bio\cache\ssl"
    if (!(Test-Path $env:BIO_CACHE_SSL_PATH)) {
        mkdir $env:BIO_CACHE_SSL_PATH | Out-Null
    }

    if (($env:CERT_PATH) -and (Test-Path $env:CERT_PATH)) {
        Write-BioInfo "Populating SSL certificate cache at $env:BIO_CACHE_SSL_PATH"
        Copy-Item -Path $env:CERT_PATH\* -Destination $env:BIO_CACHE_SSL_PATH
    }

    Set-SecretsFromEnvironment
    Update-SslCertFile

    New-PSDrive -Name "Biome" -PSProvider FileSystem -Root $BIO_STUDIO_ROOT -Scope Script | Out-Null
...
function Enter-Studio {
    ...
    $env:BIO_STUDIO_ENTER_ROOT = Resolve-Path $BIO_STUDIO_ROOT
    ...
    if (Test-InContainer) {
        ...
        if ($osVersion -lt $studio_enter_docker_min_supported_version) {
            Write-Warning "bio studio enter is not supported in Windows containers under Hyper-V isolation mode on base images of Windows prior to Windows Server 2025."
            Write-Warning "If your host container engine running on a Windows client SKU version 10 less, you may need to set `$ENV:BIO_DOCKER_OPTS to '--isolation process'."
        }
        ...
        # or check marks.
        $env:BIO_GLYPH_STYLE="ascii"
    }
    New-Studio
    $env:STUDIO_SCRIPT_ROOT = $PSScriptRoot
    $env:startedNativeStudioSup = $false
    $shouldRunSup = (!(@($false, 0, "no", "false") -contains $env:BIO_STUDIO_SUP))
    ...
    if(!(Test-InContainer) -and (Get-Process -Name bio-sup -ErrorAction SilentlyContinue)) {
        ...
        if(!$supRunningAsService) {
            ...
            mkdir $env:BIO_STUDIO_ENTER_ROOT\bio\sup\default -Force | Out-Null
            [SupervisorBootstrapper]::Run($isAnsiSupported, $env:BIO_STUDIO_SUP)
            $env:startedNativeStudioSup = $true
        } elseif($env:BIO_STUDIO_SUP) {
            [xml]$configXml = Get-Content "/bio/svc/windows-service/BioService.dll.config"
            $launcherArgs = $configXml.configuration.appSettings.SelectNodes("add[@key='launcherArgs']")[0]
            $launcherArgs.SetAttribute("value", $env:BIO_STUDIO_SUP) | Out-Null
            $configXml.Save("/bio/svc/windows-service/BioService.dll.config")
            Restart-Service Biome
        }

        Write-Host  "--> To prevent a Supervisor from running automatically in your" -ForegroundColor Cyan
        Write-Host  "    Studio, set '`$env:BIO_STUDIO_SUP=`$false' before running" -ForegroundColor Cyan
        ...
        if($null -eq $env:BIO_STUDIO_SUP) {
            Write-Host "** To pass custom arguments to run the Supervisor, set" -ForegroundColor Cyan
            Write-Host "      '`$env:BIO_STUDIO_SUP' with the arguments before running" -ForegroundColor Cyan
            Write-Host "      'bio studio enter'." -ForegroundColor Cyan
        }
        Write-Host  ""
    ...
    Write-BioInfo "Entering Studio at $BIO_STUDIO_ROOT"
    & "$PSScriptRoot\powershell\pwsh.exe" -NoProfile -ExecutionPolicy bypass -NoLogo -NoExit -Command {
        function prompt {
            Write-Host "[BIO-STUDIO]" -NoNewline -ForegroundColor Green
            " $($executionContext.SessionState.Path.CurrentLocation)$('>' * ($nestedPromptLevel +1)) "
        }
        function build {
            & "$env:STUDIO_SCRIPT_ROOT\bio-plan-build.ps1" @args
        }
        ...
        function Get-SupervisorLog {
            ...
            # spawned by bio.exe and ctrl+c behaves poorly in this scenario. So
            ...
            # a simple tail
            if (!(Test-InContainer)) {
                Start-Process "$env:STUDIO_SCRIPT_ROOT\powershell\pwsh.exe" -ArgumentList "-Command `"& {Get-Content $env:BIO_STUDIO_ENTER_ROOT\bio\sup\default\out.log -Tail 100 -Wait}`""
            } else {
                # Container studios run biome as a service which logs to a
                # configured file location
                $svcPath = Join-Path $env:SystemDrive "bio\svc\windows-service"
        ...
        function Stop-Supervisor {
            ...
            if($bioSvc -and ($bioSvc.Status -eq "Running")) {
                Stop-Service Biome
            } elseif(Test-Path "$env:BIO_STUDIO_ENTER_ROOT\bio\sup\default\LOCK") {
                Stop-Process -Id (Get-Content "$env:BIO_STUDIO_ENTER_ROOT\bio\sup\default\LOCK") -Force
                Remove-Item "$env:BIO_STUDIO_ENTER_ROOT\bio\sup\default\LOCK" -Force -ErrorAction SilentlyContinue
            }
        }

        Register-EngineEvent -SourceIdentifier PowerShell.Exiting -SupportEvent -Action {
            if($env:startedNativeStudioSup -eq $true -and (Test-Path "$env:BIO_STUDIO_ENTER_ROOT\bio\sup\default\LOCK")) {
                Stop-Process -Id (Get-Content "$env:BIO_STUDIO_ENTER_ROOT\bio\sup\default\LOCK") -Force
                $retry = 0
                while(($retry -lt 5) -and (Test-Path "$env:BIO_STUDIO_ENTER_ROOT\bio\sup\default\LOCK")) {
                    $retry += 1
        ...
        New-PSDrive -Name "Biome" -PSProvider FileSystem -Root $env:BIO_STUDIO_ENTER_ROOT | Out-Null
        Set-Location "Biome:\src"

        if((Test-Path studio_profile.ps1) -and (!$env:BIO_STUDIO_NOPROFILE)) {
            Write-Host "--> Detected and loading studio_profile.ps1"
            . .\studio_profile.ps1
        }

        # Add command line completion
        Invoke-Expression $(bio cli completers --shell powershell | Out-String)
    }
}
...
function Invoke-StudioRun($cmd) {
    ...
    New-Studio
    Write-BioInfo "Running '$cmd' in Studio at $BIO_STUDIO_ROOT"
...
function Invoke-StudioBuild($location, $reuse) {
    ...
    New-Studio
    Write-BioInfo "Building '$location' in Studio at $BIO_STUDIO_ROOT"

    & "$PSScriptRoot\bio-plan-build.ps1" $location
}
...
function Remove-Studio {
    ...
    if ($BIO_STUDIO_ROOT -eq "$env:SystemDrive\") {
        Write-BioInfo "Studio is rooted in system drive. Skipping Studio removal."
    } else {
        if(Test-Path $BIO_STUDIO_ROOT) {
            Write-BioInfo "Destroying Studio at $BIO_STUDIO_ROOT"
            ...
            cmd.exe /c RMDIR /S /Q $BIO_STUDIO_ROOT
        }
    }
}
...
function Remove-UnsafeSecret($secretList) {
    $secretList | ForEach-Object {
        if(Test-Path "env:\BIO_STUDIO_SECRET_$_") {
            Remove-Item "env:\BIO_STUDIO_SECRET_$_"
        }
    }
}

function Set-SecretsFromEnvironment {
    Remove-UnsafeSecret @('BIO_ORIGIN', 'PATH')
    Get-ChildItem env: | Where-Object { $_.Name.StartsWith('BIO_STUDIO_SECRET_') } | ForEach-Object {
        New-Item -Name $_.Name.Replace('BIO_STUDIO_SECRET_', '') -Value $_.Value -Path Env: -Force | Out-Null
...
function Update-SslCertFile {
    if($env:SSL_CERT_FILE) {
        try {
            ...
            $studio_ssl_cert_file = (Join-Path $env:BIO_CACHE_SSL_PATH $cert_filename)
            ...
            Write-BioInfo "Unable to set SSL_CERT_FILE from '$env:SSL_CERT_FILE'"
            $env:SSL_CERT_FILE = $null
        }
    }
}
...
if(!$env:BIO_STUDIOS_HOME) {
    $script:BIO_STUDIOS_HOME = "/bio/studios"
} else {
    $script:BIO_STUDIOS_HOME = $env:BIO_STUDIOS_HOME
}

if(!$env:BIO_STUDIO_ROOT) {
    $script:BIO_STUDIO_ROOT = "$BIO_STUDIOS_HOME/$dir_name"
} else {
    $script:BIO_STUDIO_ROOT = $env:BIO_STUDIO_ROOT
}

if($o) { $script:BIO_STUDIO_ROOT = $o }
$BIO_STUDIO_ROOT = $ExecutionContext.SessionState.Path.GetUnresolvedProviderPathFromPSPath($BIO_STUDIO_ROOT)

if($k) {
    $env:BIO_ORIGIN_KEYS = $k
} else {
    $env:BIO_ORIGIN_KEYS = $env:BIO_ORIGIN
}

if ((Test-Path "$env:USERPROFILE\.bio\accepted-licenses\biome") -or (Test-Path "$env:SYSTEMDRIVE\bio\accepted-licenses\biome")) {
    $env:BIO_LICENSE = "accept-no-persist"
}

if($f) { $env:BIO_REFRESH_CHANNEL = $f }
