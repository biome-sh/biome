...
set_libexec_path() {
  ...
  if [ -n "${HAB_STUDIO_BINARY:-}" ]; then
    version="$(unset HAB_STUDIO_BINARY; bio studio version | $bb cut -d ' ' -f 2)"
    libexec_path="$(unset HAB_STUDIO_BINARY; bio pkg path biome/bio-studio)/libexec"
...
# This block ensures the `HAB_LICENSE` environment variable is properly set before invoking the bio binary. 
...
if [ "${HAB_LICENSE:-}" = "accept" ] || [ "${HAB_LICENSE:-}" = "accept-no-persist" ] || [ -f "/hab/accepted-licenses/biome" ] || [ -f "$HOME/.hab/accepted-licenses/biome" ]; then
