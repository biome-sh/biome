...
new_studio() {
  ...
  *)
  ...
  $mkdir_cmd -p "$BIO_STUDIO_ROOT"
  BIO_STUDIO_ROOT="$($readlink_cmd -f "$BIO_STUDIO_ROOT")"
  ...
  # shellcheck disable=1090
  . "$studio_type_dir/bio-studio-type-darwin-${STUDIO_TYPE}.sh"
...
EOF
  ...
  if [ ! -f "$pfile" ] || ! "$grep_cmd" -q '^record() {$' "$pfile"; then
    ...
    "$cat_cmd" "$studio_profile_dir/bio-studio-darwin-profile.sh" >>"$pfile"
  fi
...
enter_studio() {
  ...
  $studio_env_command -i \
    ...
    -DWORK_DIR="$work_dir" \
    -DSTUDIO_DIR="$BIO_STUDIO_ROOT" \
    -DSTUDIO_BIO="$libexec_path/bio" \
...
load_secrets() {
  sanitize_secrets
  $env_cmd | $awk_cmd -F '=' '/^BIO_STUDIO_SECRET_/ {gsub(/BIO_STUDIO_SECRET_/, ""); print}'
}
...
build_sandbox_env() {
  ...
  if [ -f "/bio/accepted-licenses/biome" ] || [ -f "$HOME/.bio/accepted-licenses/biome" ]; then
    sandbox_env="$sandbox_env BIO_LICENSE=accept-no-persist"
  elif [ -n "${BIO_LICENSE:-}" ]; then
    sandbox_env="$sandbox_env BIO_LICENSE=$BIO_LICENSE"
  fi
  # If a Biome origin name is set, then propagate it into the Studio's
  # sandbox_environment.
  if [ -n "${BIO_ORIGIN:-}" ]; then
    sandbox_env="$sandbox_env BIO_ORIGIN=$BIO_ORIGIN"
  fi
  ...
  # If a Biome output path is set, then propagate it into the Studio's
  # sandbox_environment.
  if [ -n "${BIO_OUTPUT_PATH-}" ]; then
    sandbox_env="$sandbox_env BIO_OUTPUT_PATH=$BIO_OUTPUT_PATH"
  fi
