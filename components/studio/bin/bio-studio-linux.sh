...
#   $bb mkdir -p "$v" "$BIO_STUDIO_ROOT"/dev
...
Biome Studios - Plan for success!
...
COMMON OPTIONS:
    -a <ARTIFACT_PATH>    Sets the source artifact cache path (default: /bio/cache/artifacts)
    -c <CERT_PATH>        Sets the SSL certs cache path (default: /bio/cache/ssl)
    -f <REFRESH_CHANNEL>  Sets the channel used to retrieve plan dpendencies for Biome
                          supported origins (default: base)
    -k <BIO_ORIGIN_KEYS>  Installs secret origin keys (default:\$BIO_ORIGIN )
    -r <BIO_STUDIO_ROOT>  Sets a Studio root (default: /bio/studios/<DIR_NAME>)
...
ENVIRONMENT VARIABLES:
    ...
    BIO_NOCOLORING              Disables text coloring mode despite TERM capabilities
    BIO_NONINTERACTIVE          Disables interactive progress bars despite tty
    BIO_LICENSE                 Set to 'accept' or 'accept-no-persist' to accept the Biome license
    BIO_ORIGIN                  Propagates this variable into any studios
    BIO_ORIGIN_KEYS             Installs secret keys (\`-k' option overrides)
    BIO_PREFER_LOCAL_BIOME_DEPS  Use locally installed Biome supported dependencies if available
    BIO_STUDIOS_HOME            Sets a home path for all Studios (default: /bio/studios)
    BIO_STUDIO_NOSTUDIORC       Disables sourcing a \`.studiorc' in \`studio enter'
    BIO_STUDIO_ROOT             Sets a Studio root (\`-r' option overrides)
    BIO_STUDIO_SUP              Sets args for a Supervisor in \`studio enter'
...
Biome Studios - execute a build using a Studio
...
Biome Studios - interactively enter a Studio
...
Biome Studios - create a new Studio
...
Biome Studios - destroy a Studio
...
Biome Studios - run a command in a Studio
...
exit_if_no_studio_config() {
  if [ ! -s "$BIO_STUDIO_ROOT/.studio" ]; then
    exit_with "Directory $BIO_STUDIO_ROOT does not appear to be a Studio, aborting" 5
  fi
}
...
new_studio() {
  ...
  $bb mkdir -p "$BIO_STUDIO_ROOT"
  BIO_STUDIO_ROOT="$($bb readlink -f "$BIO_STUDIO_ROOT")"

  info "Creating Studio at $BIO_STUDIO_ROOT ($STUDIO_TYPE)"
  ...
  $bb mkdir -p $v "$BIO_STUDIO_ROOT"/dev
  # shellcheck disable=2086
  $bb mkdir -p $v "$BIO_STUDIO_ROOT"/proc
  # shellcheck disable=2086
  $bb mkdir -p $v "$BIO_STUDIO_ROOT"/sys
  # shellcheck disable=2086
  $bb mkdir -p $v "$BIO_STUDIO_ROOT"/run
  # shellcheck disable=2086
  $bb mkdir -p $v "$BIO_STUDIO_ROOT"/var/run
  ...
  # nerdy like that
  if [ -z "${NO_MOUNT}" ]; then
    if ! $bb mount | $bb grep -q "on $BIO_STUDIO_ROOT/dev type"; then
      if [ -z "${KRANGSCHNAK+x}" ]; then
        # shellcheck disable=2086
        $bb mount $v --bind /dev "$BIO_STUDIO_ROOT"/dev
      else
        # shellcheck disable=2086
        $bb mount $v --rbind /dev "$BIO_STUDIO_ROOT"/dev
      fi
    fi

    if ! $bb mount | $bb grep -q "on $BIO_STUDIO_ROOT/dev/pts type"; then
      # shellcheck disable=2086
      $bb mount $v -t devpts devpts "$BIO_STUDIO_ROOT"/dev/pts -o gid=5,mode=620
    fi
    if ! $bb mount | $bb grep -q "on $BIO_STUDIO_ROOT/proc type"; then
      # shellcheck disable=2086
      $bb mount $v -t proc proc "$BIO_STUDIO_ROOT"/proc
    fi
    if ! $bb mount | $bb grep -q "on $BIO_STUDIO_ROOT/sys type"; then
      if [ -z "${KRANGSCHNAK+x}" ]; then
        # shellcheck disable=2086
        $bb mount $v -t sysfs sysfs "$BIO_STUDIO_ROOT"/sys
      else
        # shellcheck disable=2086
        $bb mount $v --rbind /sys "$BIO_STUDIO_ROOT"/sys
      fi
    fi
    if ! $bb mount | $bb grep -q "on $BIO_STUDIO_ROOT/run type"; then
      # shellcheck disable=2086
      $bb mount $v -t tmpfs tmpfs "$BIO_STUDIO_ROOT"/run
    fi
    if [ -e /var/run/docker.sock ]; then
      if ! $bb mount | $bb grep -q "on $BIO_STUDIO_ROOT/var/run/docker.sock type"; then
        $bb touch "$BIO_STUDIO_ROOT"/var/run/docker.sock
        # shellcheck disable=2086
        $bb mount $v --bind /var/run/docker.sock "$BIO_STUDIO_ROOT"/var/run/docker.sock
      fi
    fi

    if [ -h "$BIO_STUDIO_ROOT/dev/shm" ]; then
      # shellcheck disable=2086
      $bb mkdir -p $v "$BIO_STUDIO_ROOT/$($bb readlink "$BIO_STUDIO_ROOT"/dev/shm)"
    fi

    # Mount the `$ARTIFACT_PATH` under `/bio/cache/artifacts` in the Studio,
    # unless `$NO_ARTIFACT_PATH` are set
    if [ -z "${NO_ARTIFACT_PATH}" ]; then
      studio_artifact_path="${BIO_STUDIO_ROOT}${BIO_CACHE_ARTIFACT_PATH}"
    ...
    # Mount ${CERT_PATH} under '/bio/cache/ssl' in the Studio,
    # unless `$NO_CERT_PATH` is set
    if [ -z "${NO_CERT_PATH}" ]; then
      studio_cert_path="${BIO_STUDIO_ROOT}${BIO_CACHE_CERT_PATH}"
  ...
  for top_level_dir in bin etc home lib mnt opt sbin var; do
    # shellcheck disable=2086
    $bb mkdir -p $v "$BIO_STUDIO_ROOT/$top_level_dir"
  done

  # shellcheck disable=2086
  $bb install -d $v -m 0750 "$BIO_STUDIO_ROOT/root"
  # shellcheck disable=2086
  $bb install -d $v -m 1777 "$BIO_STUDIO_ROOT/tmp" "$BIO_STUDIO_ROOT/var/tmp"

  for usr_dir in bin include lib libexec sbin; do
    # shellcheck disable=2086
    $bb mkdir -p $v "$BIO_STUDIO_ROOT/usr/$usr_dir"
  done

  for usr_share_dir in doc info locale man misc terminfo zoneinfo; do
    # shellcheck disable=2086
    $bb mkdir -p $v "$BIO_STUDIO_ROOT/usr/share/$usr_share_dir"
  done

  for usr_share_man_dir_num in 1 2 3 4 5 6 7 8; do
    # shellcheck disable=2086
    $bb mkdir -p $v "$BIO_STUDIO_ROOT/usr/share/man/man$usr_share_man_dir_num"
  done
  # If the system is 64-bit, a few symlinks will be required
  case $($bb uname -m) in
  x86_64)
    # shellcheck disable=2086
    $bb ln -sf $v lib "$BIO_STUDIO_ROOT/lib64"
    # shellcheck disable=2086
    $bb ln -sf $v lib "$BIO_STUDIO_ROOT/usr/lib64"
    ;;
  esac

  for var_dir in log mail spool opt cache local; do
    # shellcheck disable=2086
    $bb mkdir -p $v "$BIO_STUDIO_ROOT/var/$var_dir"
  done

  # shellcheck disable=2086
  $bb ln -sf $v /run/lock "$BIO_STUDIO_ROOT/var/lock"

  # shellcheck disable=2086
  $bb mkdir -p $v "$BIO_STUDIO_ROOT/var/lib/color"
  # shellcheck disable=2086
  $bb mkdir -p $v "$BIO_STUDIO_ROOT/var/lib/misc"
  # shellcheck disable=2086
  $bb mkdir -p $v "$BIO_STUDIO_ROOT/var/lib/locate"

  # shellcheck disable=2086
  $bb ln -sf $v /proc/self/mounts "$BIO_STUDIO_ROOT/etc/mtab"

  # Load the appropriate type strategy to complete the setup
  if [ -n "${BIO_STUDIO_BINARY:-}" ]; then
    studio_type_dir="$studio_binary_libexec_path"
  else
    studio_type_dir="$libexec_path"
  fi
  # shellcheck disable=1090
  . "$studio_type_dir/bio-studio-type-${STUDIO_TYPE}.sh"
...
EOF
  ...
  if [ "$STUDIO_TYPE" != "bare" ]; then
    pfile="$BIO_STUDIO_ROOT/etc/profile"
    if [ ! -f "$pfile" ] || ! $bb grep -q '^record() {$' "$pfile"; then
      ...
      $bb cat "$studio_profile_dir/bio-studio-profile.sh" >> "$pfile"

    fi

    # shellcheck disable=2086
    $bb mkdir -p $v "$BIO_STUDIO_ROOT/src"
    # Mount the `$SRC_PATH` under `/src` in the Studio, unless either `$NO_MOUNT`
    # or `$NO_SRC_PATH` are set
    if [ -z "${NO_MOUNT}" ] && [ -z "${NO_SRC_PATH}" ]; then
      if ! $bb mount | $bb grep -q "on $BIO_STUDIO_ROOT/src type"; then
        # shellcheck disable=2086
        $bb mount $v --bind "$SRC_PATH" "$BIO_STUDIO_ROOT/src"
      fi
    fi
  fi
}
...
enter_studio() {
  ...
  info "Entering Studio at $BIO_STUDIO_ROOT ($STUDIO_TYPE)"
  report_env_vars
  echo
  ...
  # shellcheck disable=2086
  $bb chroot "$BIO_STUDIO_ROOT" "$studio_env_command" -i $env $studio_enter_command "$@"
}
...
build_studio() {
  ...
  if [ -z "$studio_build_command" ]; then
    exit_with "Studio at $BIO_STUDIO_ROOT ($STUDIO_TYPE) does not support 'build'" 10
  fi

  env="$(chroot_env "$studio_path" "$studio_build_environment")"

  info "Building '$*' in Studio at $BIO_STUDIO_ROOT ($STUDIO_TYPE)"
  report_env_vars
  ...
  # shellcheck disable=2086
  echo $studio_build_command "$@" | $bb chroot "$BIO_STUDIO_ROOT" "$studio_env_command" -i $env ${studio_run_command:?}
}
...
run_studio() {
  ...
  info "Running '$*' in Studio at $BIO_STUDIO_ROOT ($STUDIO_TYPE)"
  ...
  # shellcheck disable=2086
  echo "$@" | $bb chroot "$BIO_STUDIO_ROOT" "$studio_env_command" -i $env $studio_run_command
}
...
rm_studio() {
  ...
  if [ -d "$BIO_STUDIO_ROOT" ]; then
    # Properly canonicalize the root path of the Studio by following all symlinks.
    BIO_STUDIO_ROOT="$($bb readlink -f "$BIO_STUDIO_ROOT")"
  fi

  info "Destroying Studio at $BIO_STUDIO_ROOT ($STUDIO_TYPE)"

  cleanup_studio

  if $bb mount | $bb grep -q "on ${BIO_STUDIO_ROOT}.* type"; then
...
remaining mounted filesystems under ${BIO_STUDIO_ROOT}:"
    $bb mount \
      | $bb grep "on ${BIO_STUDIO_ROOT}.* type" \
      | $bb sed 's#^#    * #'
    >&2 echo "Unmount these remaining filesystem using \`umount(8)'and retry \
the last command."
    exit_with "Remaining mounted filesystems found under $BIO_STUDIO_ROOT" \
      "$ERR_REMAINING_MOUNTS"
  else
    # No remaining mounted filesystems, so remove remaining files and dirs
    # shellcheck disable=2086
    $bb rm -rf $v "$BIO_STUDIO_ROOT"
  fi
}
...
ensure_root() {
  ...
  # Otherwise, prepare to die with message formatting similar to the `bio` program.
  warn_msg="Running Biome Studio requires root or administrator privileges. \
...
# **Internal** Builds up a secret environment based on the prefix `BIO_STUDIO_SECRET_`
# to pass into the studio
load_secrets() {
  sanitize_secrets
  $bb env | $bb awk -F '=' '/^BIO_STUDIO_SECRET_/ {gsub(/BIO_STUDIO_SECRET_/, ""); print}'
}
...
chroot_env() {
  ...
  # session.
  env="LC_ALL=POSIX HOME=/root TERM=${TERM:-} PATH=$studio_path BIO_LICENSE=$BIO_LICENSE"
  ...
  # If a Biome config filetype ignore string is set, then propagate it
  # into the Studio's environment.
  if [ -n "${BIO_CONFIG_EXCLUDE:-}" ]; then
    env="$env BIO_CONFIG_EXCLUDE=$BIO_CONFIG_EXCLUDE"
  fi
  # If a Biome Auth Token is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_AUTH_TOKEN:-}" ]; then
    env="$env BIO_AUTH_TOKEN=$BIO_AUTH_TOKEN"
  fi
  # If a Biome Builder URL is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_BLDR_URL:-}" ]; then
    env="$env BIO_BLDR_URL=$BIO_BLDR_URL"
  fi
  # If a Biome Depot Channel is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_BLDR_CHANNEL:-}" ]; then
    env="$env BIO_BLDR_CHANNEL=$BIO_BLDR_CHANNEL"
  fi
  # If a Biome refresh Channel is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_REFRESH_CHANNEL:-}" ]; then
    env="$env BIO_REFRESH_CHANNEL=$BIO_REFRESH_CHANNEL"
  fi
  # If a no coloring environment variable is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_NOCOLORING:-}" ]; then
    env="$env BIO_NOCOLORING=$BIO_NOCOLORING"
  fi
  # If a noninteractive environment variable is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_NONINTERACTIVE:-}" ]; then
    env="$env BIO_NONINTERACTIVE=$BIO_NONINTERACTIVE"
  fi
  # If a Biome origin name is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_ORIGIN:-}" ]; then
    env="$env BIO_ORIGIN=$BIO_ORIGIN"
  fi
  ...
  if [ -n "${BIO_STUDIO_NOSTUDIORC:-}" ]; then
    env="$env BIO_STUDIO_NOSTUDIORC=$BIO_STUDIO_NOSTUDIORC"
  fi
  ...
  if [ -n "${BIO_STUDIO_SUP:-}" ]; then
    ...
    env="$env BIO_STUDIO_SUP=$(echo "$BIO_STUDIO_SUP" | $bb sed 's/ /__sp__/g')"
  fi
  # If a Biome update strategy frequency is set, then propagate it into the
  # Studio's environment.
  if [ -n "${BIO_UPDATE_STRATEGY_PERIOD_MS:-}" ]; then
    env="$env BIO_UPDATE_STRATEGY_PERIOD_MS=$BIO_UPDATE_STRATEGY_PERIOD_MS"
  fi
  # If a Biome studio binary is set, then propagate it into the Studio's
  # environment.
  if [ -n "${BIO_STUDIO_BINARY:-}" ]; then
    env="$env BIO_STUDIO_BINARY=$BIO_STUDIO_BINARY"
  fi
  ...
  && [ -z "${NO_MOUNT}" ] \
  && [ -z "${NO_CERT_PATH}" ]; then
    studio_ssl_cert_file="${BIO_CACHE_CERT_PATH}/$($bb basename "$SSL_CERT_FILE")"
    # Only set SSL_CERT_FILE inside the studio if the file is present in the cache
    if [ -f "${BIO_STUDIO_ROOT}/${studio_ssl_cert_file}" ]; then
...
report_env_vars() {
  ...
  if [ -n "${BIO_REFRESH_CHANNEL:-}" ]; then
    info "Exported: BIO_REFRESH_CHANNEL=$BIO_REFRESH_CHANNEL"
  fi
...
kill_launcher() {
  pid_file="$BIO_STUDIO_ROOT/bio/sup/default/LOCK"
...
unmount_filesystems() {
  # shellcheck disable=2086
  umount_fs $v -l "$BIO_STUDIO_ROOT/src"
  ...
  # shellcheck disable=2086
  umount_fs $v "$BIO_STUDIO_ROOT/run"

  if [ -z "${KRANGSCHNAK+x}" ]; then
    # shellcheck disable=2086
    umount_fs $v "$BIO_STUDIO_ROOT/sys"
  else
    # shellcheck disable=2086
    umount_fs $v -l "$BIO_STUDIO_ROOT/sys"
  fi

  # shellcheck disable=2086
  umount_fs $v "$BIO_STUDIO_ROOT/proc"

  # shellcheck disable=2086
  umount_fs $v "$BIO_STUDIO_ROOT/dev/pts"

  # shellcheck disable=2086
  umount_fs $v -l "$BIO_STUDIO_ROOT/dev"

  # shellcheck disable=2086
  umount_fs $v -l "$BIO_STUDIO_ROOT/var/run/docker.sock"
}
...
set_libexec_path() {
  ...
  if [ -n "${BIO_STUDIO_BINARY:-}" ]; then
    version="$(unset BIO_STUDIO_BINARY; bio studio version | $bb cut -d ' ' -f 2)"
    libexec_path="$(unset BIO_STUDIO_BINARY; bio pkg path biome/bio-studio)/libexec"
    studio_binary_libexec_path="$($bb dirname "$BIO_STUDIO_BINARY")/../libexec"
  else
    p=$($bb dirname "$0")
...
copy_minimal_default_file_if_not_present() {
    file_path="${1}"
    if [ -f "${BIO_STUDIO_ROOT}${file_path}" ]; then
        if [ -n "$VERBOSE" ]; then
...
copy_minimal_default_file() {
    ...
    if [ -f "${defaults_path}${file_path}" ]; then
        $bb cp -f "${defaults_path}${file_path}" "${BIO_STUDIO_ROOT}${file_path}"
    else
...
BIO_PKG_PATH=$BIO_ROOT_PATH/pkgs
# The default download root path for package artifacts, used on package
# installation
BIO_CACHE_ARTIFACT_PATH=$BIO_ROOT_PATH/cache/artifacts

# The default root path for SSL certs
BIO_CACHE_CERT_PATH=$BIO_ROOT_PATH/cache/ssl

# This block ensures the `BIO_LICENSE` environment variable is properly set before invoking the bio binary.
# It first checks if `BIO_LICENSE` is already 'accept' or 'accept-no-persist'. If not, it searches for an
# acceptance file ('biome') in '/bio/accepted-licenses' and '$HOME/.bio/accepted-licenses'.
# If none of these conditions are met, it assigns 'deny' to `BIO_LICENSE`. This prevents any interactive
# license acceptance prompts from halting the studio operation by ensuring `BIO_LICENSE` is always either
# accepted or denied prior to each bio invocation.
if [ "${BIO_LICENSE:-}" = "accept" ] || [ "${BIO_LICENSE:-}" = "accept-no-persist" ] || [ -f "/bio/accepted-licenses/biome" ] || [ -f "$HOME/.bio/accepted-licenses/biome" ]; then
  BIO_LICENSE="${BIO_LICENSE:-accept-no-persist}"
else
  BIO_LICENSE="deny"
fi
...
while getopts ":nNMa:c:f:k:r:s:t:D:vqVh" opt; do
  case $opt in
    ...
    c)
      ...
      ;;
    f)
      export BIO_REFRESH_CHANNEL="$OPTARG"
      ;;
    n)
      NO_SRC_PATH=true
      ;;
    ...
    M)
      ...
      ;;
    k)
      BIO_ORIGIN_KEYS=$OPTARG
      ;;
    r)
      BIO_STUDIO_ROOT=$OPTARG
      ;;
    s)
      SRC_PATH=$OPTARG
      ;;
