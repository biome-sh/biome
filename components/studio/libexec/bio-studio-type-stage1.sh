...
: "${STAGE1_TOOLS_URL:=http://s3-us-west-2.amazonaws.com/biome-studio-stage1/biome-studio-stage1-20180312233639.tar.xz}"
: "${TAR_DIR:=/tmp}"
...
finish_setup() {
  ...
  if [ -x "$BIO_STUDIO_ROOT/tools/bin/bash" ]; then
    return 0
  fi
  ...
  $bb xzcat "$tar_file" | $bb tar xf - -C "$BIO_STUDIO_ROOT"
  ...
  {
  # shellcheck disable=2154
  $bb ln -sf $v /tools/bin/bash "$BIO_STUDIO_ROOT"/bin
  $bb ln -sf $v /tools/bin/cat "$BIO_STUDIO_ROOT"/bin
  $bb ln -sf $v /tools/bin/echo "$BIO_STUDIO_ROOT"/bin
  $bb ln -sf $v /tools/bin/pwd "$BIO_STUDIO_ROOT"/bin
  $bb ln -sf $v /tools/bin/stty "$BIO_STUDIO_ROOT"/bin

  $bb ln -sf $v /tools/bin/perl "$BIO_STUDIO_ROOT"/usr/bin
  $bb ln -sf $v /tools/lib/libgcc_s.so "$BIO_STUDIO_ROOT"/usr/lib
  $bb ln -sf $v /tools/lib/libgcc_s.so.1 "$BIO_STUDIO_ROOT"/usr/lib
  $bb ln -sf $v /tools/lib/libstdc++.so "$BIO_STUDIO_ROOT"/usr/lib
  $bb ln -sf $v /tools/lib/libstdc++.so.6 "$BIO_STUDIO_ROOT"/usr/lib    
  ...
  # step.
  if [ -f "$BIO_STUDIO_ROOT/tools/lib/libstdc++.la" ]; then
    $bb sed 's/tools/usr/' "$BIO_STUDIO_ROOT"/tools/lib/libstdc++.la \
      > "$BIO_STUDIO_ROOT"/usr/lib/libstdc++.la
  fi
  # shellcheck disable=2086
  $bb ln -sf $v bash "$BIO_STUDIO_ROOT"/bin/sh

  # Set the login shell for any relevant user to be `/bin/bash`
  $bb sed -e 's,/bin/sh,/bin/bash,g' -i "$BIO_STUDIO_ROOT"/etc/passwd
