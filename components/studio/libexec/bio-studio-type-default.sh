...
studio_enter_command="$HAB_ROOT_PATH/bin/bio pkg exec biome/bio-backline bash --login +h"
...
#!$bash_path/bin/sh
exec $HAB_ROOT_PATH/bin/bio pkg exec biome/bio-plan-build bio-plan-build "\$@"
EOF
...
sup-run() {
  ...
  setsid bio sup run "\$@" > /hab/sup/default/sup.log 2>&1 &
  disown \$!
