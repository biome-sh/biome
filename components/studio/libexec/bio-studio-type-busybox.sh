...
finish_setup() {
  ...
  $bb mkdir -p $v "$BIO_STUDIO_ROOT"/opt/busybox
  # shellcheck disable=2154,2086
  $bb cp $v "$libexec_path"/busybox "$BIO_STUDIO_ROOT"/opt/busybox/

  if [ ! -f "$BIO_STUDIO_ROOT/opt/busybox/sh" ]; then
    # Symlink all tools to busybox under `/opt/busybox`
    for c in $("$BIO_STUDIO_ROOT"/opt/busybox/busybox --list); do
      # shellcheck disable=2086
      $bb ln -sf $v busybox "$BIO_STUDIO_ROOT"/opt/busybox/$c
    done
  fi

  # Set the login shell for any relevant user to be busybox's `sh`
  $bb sed -e 's,/bin/sh,/opt/busybox/sh,g' -i "$BIO_STUDIO_ROOT"/etc/passwd
}
