...
async fn test_for_restart_on_config_application(test_name: &str,
    ...
    let mut test_sup =
        utils::TestSup::new_with_random_ports(&bio_root,
...
async fn package_with_successful_install_hook_in_dependency_is_loaded() -> Result<()> {
    ...
    let initial_dependency_snapshot =
        FileSystemSnapshot::new(bio_root.pkg_dir_path(origin_name, dependency_name)
                                        .as_path()).await?;
    ...
    let final_dependency_snapshot = FileSystemSnapshot::new(bio_root.pkg_dir_path(origin_name,
                                                                                  dependency_name)
                                                                    .as_path()).await?;
