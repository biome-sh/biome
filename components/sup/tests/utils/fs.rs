...
pub async fn setup_package_files(origin_name: &str,
    ...
    copy_dir(&expanded_fixture_dir, &bio_pkg_path).await
                                                  .with_context(|| {
    ...
    write_default_metafiles(bio_root, &origin_name, &package_name).await
    ...
    let install =
        PackageInstall::load(&bio_root.pkg_ident(&origin_name, &package_name),
                             Some(bio_root.as_ref())).with_context(|| {
    ...
    if let Ok(tdeps) = install.tdeps() {
        for dependency in tdeps.iter() {
            ...
            write_default_metafiles(bio_root, &dependency.origin, &dependency.name).await.context("Failed to write meta files for native package")?;
        }
    }
...
async fn write_default_metafiles(bio_root: &BioRoot,
                                 pkg_origin: &str,
                                 pkg_name: &str)
                                 -> Result<()> {
