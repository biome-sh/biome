...
pub async fn setup_package_files(origin_name: &str,
    ...
    write_default_svc_user_and_group_metafiles(bio_root, &origin_name, &package_name).await.context("Failed to write default files for service")?;
...
async fn write_default_svc_user_and_group_metafiles(bio_root: &BioRoot,
                                                    pkg_origin: &str,
                                                    pkg_name: &str)
                                                    -> Result<()> {
