...
use biome_core::{crypto::Blake2bHash, package::PackageInstall, users};
use std::{
    ...
    pub fn modifications_since(&self, other: &FileSystemSnapshot, exclude: &[Pattern]) -> FileSystemModifications {
        ...
        FileSystemModifications {
            added: self
                ...
                .filter_map(|f| f.path.strip_prefix(self.path.as_path()).unwrap().to_str())
                .filter_map(|x| {
            ...
            updated: self
                ...
                .iter()
                .filter_map(|f| {
                    if let Some(old) = other.files.iter().find(|of| of.path == f.path) {
                        if old.hash != f.hash {
                            f.path
...
impl FileSnapshot {
    pub fn new(path: PathBuf) -> Result<FileSnapshot> {
        Ok(FileSnapshot {
            ...
            hash: Blake2bHash::from_file(&path).context(
                "Failed to hash file \
                                                                       contents",
            )?,
            path,
    ...
    pub async fn await_update(&self, timeout: Duration) -> Result<FileSnapshot> {
        ...
        loop {
            if timeout.saturating_sub(started_at.elapsed()) == Duration::ZERO {
                return Err(anyhow!(
                    "Timed out waiting for file '{}' to be updated",
                    self.path.display()
                ));
            }
...
pub async fn setup_package_files(
    ...
    if let Ok(tdeps) = install.tdeps() {
        for dependency in tdeps.iter() {
            ...
            let pkg_path = bio_root.pkg_dir_path(&dependency.origin, &dependency.name);
            copy_dir(&fixture_dir, &pkg_path).await.with_context(|| {
                format!(
                    "Failed to copy transitive \
                                                                 dependency directory '{}' to '{}'",
                    fixture_dir.display(),
                    pkg_path.display()
                )
            })?;
            write_default_metafiles(bio_root, &dependency.origin, &dependency.name)
                .await
                .context("Failed to write meta files for native package")?;
        }
    }

    // Ensure the directory for the spec files exists
    let spec_dir = bio_root.spec_dir_path(&service_group);
    ...
    D: AsRef<Path>,
{
    ...
    while let Some((source_dir, dest_dir)) = queue.pop_front() {
        ...
        let mut source_dir_entries = fs::read_dir(&source_dir)
            .await
            .with_context(|| format!("Could not read entries in '{}'", source_dir.display()))?;
        while let Some(entry) = source_dir_entries.next_entry().await.with_context(|| {
            format!(
                "Failed to read entries for \
                                                               directory '{}'",
                source_dir.display()
            )
        })? {
...
async fn write_default_metafiles(bio_root: &BioRoot, pkg_origin: &str, pkg_name: &str) -> Result<()> {
    ...
    if !svc_group_metafile.is_file() {
        write_metafile(
            ...
            users::get_current_groupname()
                .context("Failed to get groupname")?
    ...
    {
        let pkg_type_metafile = bio_root.pkg_type_path(pkg_origin, pkg_name);
