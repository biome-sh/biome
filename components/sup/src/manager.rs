...
use self::{
    action::{ShutdownInput, SupervisorAction},
...
use crate::{
    ...
    ctl_gateway::{self, CtlRequest, acceptor::CtlAcceptor, server::CtlGatewayServer},
    error::{Error, Result},
    event::{self, EventStreamConfig},
    http_gateway,
    lock_file::LockFile,
    util::pkg,
};
use biome_butterfly::{
    member::Member,
    server::{ServerProxy, Suitability, timing::Timing},
};
use biome_common::{
    FeatureFlag, liveliness_checker, outputln,
    types::{GossipListenAddr, HttpListenAddr, ListenCtlAddr},
};
#[cfg(unix)]
use biome_core::os::{
    process::{ShutdownSignal, Signal},
    signals,
};
use biome_core::{
    ChannelIdent,
    ...
    env::Config,
    fs::FS_ROOT_PATH,
    os::process::{self, ShutdownTimeout},
    package::{Identifiable, PackageIdent, PackageInstall},
    service::ServiceGroup,
    tls::rustls_wrapper::{certificates_from_file, private_key_from_file},
    util::ToI64,
};
use biome_launcher_client::{LauncherCli, LauncherStatus};
use biome_sup_protocol::{self};
use cpu_time::ProcessTime;
use futures::{
    channel::{mpsc as fut_mpsc, oneshot},
    future,
    prelude::*,
...
lazy_static! {
    ...
    /// produces a fully qualified ident when built with Biome. If it is built directly from
...
impl Default for ServiceRestartConfig {
    fn default() -> Self {
        Self {
            min_backoff_period: Default::default(),
...
impl From<PrivatePkcs8KeyDer<'static>> for CloneablePkcs8PrivKey {
    fn from(k: PrivatePkcs8KeyDer<'static>) -> Self {
        Self(k)
    }
}
...
pub(crate) mod sync {
    ...
    impl<'a> GatewayStateReadGuard<'a> {
        fn new(lock: &'a Lock<GatewayStateInner>) -> Self {
            Self(lock.read())
        }

        pub fn butterfly_data(&self) -> &str {
            &self.0.butterfly_data
        }

        pub fn census_data(&self) -> &str {
            &self.0.census_data
        }

        pub fn services_data(&self) -> &[ServiceQueryModel] {
            self.0.services_data.as_slice()
        }
    }
    ...
    impl<'a> GatewayStateWriteGuard<'a> {
        ...
        pub fn set_census_data(&mut self, new_data: String) {
            self.0.census_data = new_data
        }

        pub fn set_butterfly_data(&mut self, new_data: String) {
            self.0.butterfly_data = new_data
        }
    ...
    impl<'a> ManagerServicesReadGuard<'a> {
        fn new(lock: &'a Lock<ManagerServicesInner>) -> Self {
            Self(lock.read())
        }
...
impl Manager {
    ...
    async fn new_imlw(
        ...
        let server = biome_butterfly::Server::new(
            sys.gossip_listen(),
            sys.gossip_listen(),
            member,
            cfg.ring_key,
        ...
        Ok(Manager {
            ...
            self_updater,
            service_updater: Arc::new(Mutex::new(ServiceUpdater::new(
                server.clone(),
    ...
    async fn maybe_uninstall_old_packages(&self, ident: &PackageIdent) {
        if let Some(number_latest_to_keep) = self.state.cfg.keep_latest_packages {
            match pkg::uninstall_all_but_latest(ident, number_latest_to_keep).await {
                Ok(uninstalled) => {
                    info!(
                        "Uninstalled '{}' '{}' packages keeping the '{}' latest",
                        uninstalled, ident, number_latest_to_keep
                    )
                }
                Err(e) => {
                    error!(
                        "Failed to uninstall '{}' packages keeping the '{}' latest, err: {}",
    ...
    async fn add_service_rsw_mlw_rhw_msr(&mut self, spec: ServiceSpec) {
        ...
        self.state.services.lock_msw().insert(
            service.spec_ident(),
    ...
    pub async fn run_rsw_imlw_mlw_gsw_smw_rhw_msw(
        mut self,
        svc_load_msgs: Vec<biome_sup_protocol::ctl::SvcLoad>,
    ) -> Result<()> {
        ...
        self.census_ring.write().update_from_rumors_rsr_mlr(
            &self.state.cfg.key_cache,
        ...
        let ctl_gateway_server = CtlGatewayServer {
            listen_addr: self.sys.ctl_listen(),
        ...
        let shutdown_mode = loop {
            ...
            if self.feature_flags.contains(FeatureFlag::TEST_EXIT)
                && let Ok(exit_file_path) = env::var("BIO_FEAT_TEST_EXIT")
                ...
                match action {
                    StopService {
                        ...
                        if let Err(err) = self.state.cfg.save_spec_for(&service_spec) {
                            warn!(
                                "Tried to stop '{}', but couldn't update the spec: {:?}",
                    ...
                    UpdateService { service_spec } => {
                        ...
                        if let Err(err) = self.state.cfg.save_spec_for(&service_spec) {
                            warn!(
                                "Tried to update '{}', but couldn't write the spec: {:?}",
                                service_spec.ident, err
                            );
                        }
                    }
            ...
            ShutdownMode::Normal | ShutdownMode::Departed => {
                ...
                let service_stop_futures = FuturesUnordered::from_iter(
                    self.state
                        ...
                        .lock_msw()
                        .drain_services()
                        .map(|service| self.stop_service_future_gsw(service, None, None)),
                );
    ...
    fn gossip_latest_service_rumor_rsw_mlw_rhw(&self, service: &Service, updated_pkg_incarnation: Option<u64>) {
        let (incarnation, last_pkg_incarnation) = self
            ...
            .map_rumor(&self.sys.member_id, |rumor| {
                (rumor.incarnation + 1, rumor.pkg_incarnation)
            })
            .unwrap_or((1, 0));
        ...
        let mut services_data: Vec<ServiceQueryModel> = service_map
            ...
            .filter_map(|(_, svc_state)| {
                if let Some(service) = svc_state.service() {
                    return Some(ServiceQueryModel::new(
                        service,
    ...
    fn stop_service_gsw_msw(&mut self, ident: &PackageIdent, shutdown_input: &ShutdownInput) {
        match self.remove_service_from_state_msw(ident) {
            ...
            _ => {
                warn!(
                    "Tried to stop '{}', but couldn't find it in our list of running services!",
                    ident
                );
            }
        }
    ...
    async fn uninstall_newer_packages(install_ident: &PackageIdent, latest_desired_ident: &PackageIdent) {
        while let Some(latest_installed) = pkg::installed(install_ident) {
            ...
            if latest_ident > *latest_desired_ident {
                info!(
                    "Uninstalling '{}' inorder to ensure '{}' is the latest installed package",
                    latest_ident, latest_desired_ident
                );
                if let Err(e) = pkg::uninstall_even_if_loaded(&latest_ident).await {
                    error!(
                        "Failed to uninstall '{}' unable to ensure '{}' is the latest \
    ...
    async fn wrap_async_service_operation<F>(
        ident: PackageIdent,
        busy_services: Arc<Mutex<HashSet<PackageIdent>>>,
        services_need_reconciliation: ReconciliationFlag,
        fut: F,
        ...
        for op in ops.into_iter() {
            match op {
                ServiceOperation::Restart { to_stop: spec, .. } | ServiceOperation::Stop(spec) => {
                    ...
                    // restart future.
                    match self
                        .remove_service_from_state_msw(&spec.ident)
                        .and_then(|mut service_state| service_state.shutdown(false))
                    {
                        ...
                        _ => {
                            ...
                            outputln!(
                                "Tried to remove service for {} but could not find it \
                                       running, skipping",
                                &spec.ident
                            );
                        }
                ...
                ServiceOperation::Update(spec, ops) => {
                        ...
                        .and_then(PersistentServiceWrapper::service_mut)
                    {
                        ...
                        outputln!(
                            "Tried to update config for service {} but could not find it \
                                   running, skipping",
                            &spec.ident
                        );
                    }
...
mod tests {
    ...
    fn manager_state_path_custom() {
        let cfg = ManagerConfig {
            custom_state_path: Some(PathBuf::from("/tmp/peanuts-and-cake")),
            ..Default::default()
        };
        let path = cfg.sup_root();
    ...
    fn manager_state_path_custom_beats_name() {
        let cfg = ManagerConfig {
            custom_state_path: Some(PathBuf::from("/tmp/partay")),
            ..Default::default()
        };
        let path = cfg.sup_root();
