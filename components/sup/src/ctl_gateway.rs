...
use biome_common::{
    PROGRAM_NAME,
    output::{self, OutputContext, OutputFormat, StructuredOutput},
    ui::UIWriter,
};
use log::warn;
use std::{
...
impl Write for CtlRequest {
    ...
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        let msg = biome_sup_protocol::ctl::ConsoleLine {
            line: line.to_string(),
            color: color_to_string(self.current_color_spec.fg()),
...
impl NetProgressBar {
    ...
    pub fn new(req: CtlRequest) -> Self {
        NetProgressBar {
            inner: biome_sup_protocol::ctl::NetProgress::default(),
            req,
        }
    }
}
