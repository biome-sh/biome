...
use bio::{
    command::pkg::{
        self,
        uninstall::{self, UninstallHookMode, UninstallSafety},
    },
...
use biome_core::{
    AUTH_TOKEN_ENVVAR, ChannelIdent, env as henv,
...
pub async fn satisfy_or_install<T>(
    ui: &mut T,
    install_source: &InstallSource,
    bldr_url: &str,
    channel: &ChannelIdent,
...
pub async fn install_channel_head(
    url: &str,
    ...
    let channel_latest_ident = api_client
        .show_package(
            (ident.as_ref(), PackageTarget::active_target()),
            channel,
            auth_token.as_deref(),
        )
        .await?;
...
pub async fn uninstall_all_but_latest(
    ident: impl AsRef<PackageIdent>,
    number_latest_to_keep: usize,
) -> BioResult<usize> {
    uninstall::uninstall_all_but_latest(
        &mut NullUi::new(),
        ident,
