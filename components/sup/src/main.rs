...
async fn start_rsr_imlw_mlw_gsw_smw_rhw_msw(feature_flags: FeatureFlag) -> Result<()> {
    ...
    match app_matches.subcommand() {
        ...
        ("run", Some(_)) => {
            // TODO (DM): This is a little hacky. Essentially, for `bio sup run` we switch to using
...
mod test {
    ...
    use biome_common::types::{GossipListenAddr,
                                HttpListenAddr,
                                ListenCtlAddr};
    ...
    mod manager_config {
        ...
        use biome_common::types::EventStreamConnectMethod;
        #[cfg(windows)]
...
"#;
        ...
        fn test_bio_sup_run_cli_1() {
            ...
            let args = format!("bio-sup run --listen-gossip=1.2.3.4:4321 \
        ...
        fn test_bio_sup_run_cli_event_stream() {
            ...
            let args = format!("bio-sup run --event-stream-application=MY_APP \
        ...
        fn test_bio_sup_run_cli_svc() {
            ...
            let args = format!("bio-sup run --channel my_channel --bind one:service1.default \
...
"#;
            ...
            let args = format!("bio-sup run --config-files {} {} --listen-http 3.3.3.3:3333",
        ...
        fn test_bio_sup_run_all_possible_values() {
            let args = "bio-sup run --topology standalone --strategy none --update-condition \
