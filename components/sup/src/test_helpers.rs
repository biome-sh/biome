...
pub fn assert_valid(json_string: &str, schema: &str) {
    ...
    if !result.is_valid() {
        let error_string = result
            ...
            .join("\n");
        let pretty_json =
            serde_json::to_string_pretty(&serde_json::from_str::<serde_json::Value>(json_string).expect(
                "JSON should \
                                                                                  parse if we \
                                                                                  get this far",
            ))
...
pub fn validate_string(input: &str, schema: &str) -> ValidationResult {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("doc").join(schema);
    let validator = build_validator(&path);
    let input_json: serde_json::Value = serde_json::from_str(input).expect("Could not parse input as JSON");
    let errors = validator.iter_errors(&input_json).map(|e| e.to_string()).collect();
...
fn build_validator(path: &Path) -> jsonschema::Validator {
    ...
    for entry in fs::read_dir(schema_dir).expect("could not read schema directory") {
        let entry = entry.expect("could not read schema directory entry");
        ...
        let sibling: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(&sibling_path).expect("could not read sibling schema"))
        ...
        if let Some(sibling_defs) = sibling.get("definitions").and_then(|v| v.as_object()) {
            ...
            let root_defs_map = root_defs.as_object_mut().expect("'definitions' must be a JSON object");
    ...
    match val {
        serde_json::Value::Object(obj) => {
            if let Some(ref_val) = obj.get("$ref").and_then(|v| v.as_str()).map(str::to_owned)
                ...
                && !file.is_empty()
            {
                obj.insert("$ref".to_string(), serde_json::Value::String(format!("#{ptr}")));
            }
