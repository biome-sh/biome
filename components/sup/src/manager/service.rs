...
use self::{
    ...
    hooks::{HookCompileTable, HookTable, HookTableQueryModel},
    supervisor::{PidUpdate, SupervisedProcessQueryModel, Supervisor},
};
pub use self::{
    health::{HealthCheckBundle, HealthCheckHookStatus, HealthCheckResult},
    hooks::{HealthCheckHook, ProcessOutput, StandardStreams},
...
pub use biome_common::templating::{
    config::{Cfg, UserConfigPath},
    package::{Env, Pkg},
};
use biome_common::{
    FeatureFlag, outputln,
    templating::{config::CfgRenderer, hooks::Hook, package::PkgQueryModel},
};
#[cfg(windows)]
...
use biome_core::{
    ...
    crypto::Blake2bHash,
    flowcontrol::Backoff,
    fs::{FS_ROOT_PATH, SvcDir, atomic_write, svc_hooks_path},
    os::process::{Pid, ShutdownTimeout},
    package::{PackageIdent, PackageInstall, metadata::Bind},
...
pub use biome_sup_protocol::types::{ProcessState, Topology, UpdateCondition, UpdateStrategy};
...
impl TemplateUpdate {
    ...
    fn needs_reconfigure(&self) -> bool {
        self.config_changed || self.hooks.reconfigure_changed()
    }
}
...
impl ServiceRunState {
    pub fn new(restart_config: &ServiceRestartConfig) -> ServiceRunState {
        ServiceRunState {
            ...
            restart_backoff: Backoff::new(
                restart_config.min_backoff_period,
    ...
    pub fn mark_for_immediate_restart(
        &mut self,
        old_pid: Option<Pid>,
...
impl PersistentServiceWrapper {
    ...
    pub fn service_run_state(&self) -> &ServiceRunState {
        &self.run_state
    }

    pub fn service_run_state_mut(&mut self) -> &ServiceRunState {
        &mut self.run_state
    }

    /// Mark this service for an immediate restart
    pub fn mark_for_restart_due_to_update(&mut self, timestamp: SystemTime) {
        self.run_state.mark_for_immediate_restart(
            self.run_state.current_pid,
...
impl Service {
    pub(crate) fn bldr_url(&self) -> String {
        self.spec.bldr_url.clone()
    }

    pub(crate) fn channel(&self) -> ChannelIdent {
        self.spec.channel.clone()
    }

    pub(crate) fn spec_ident(&self) -> PackageIdent {
        self.spec.ident.clone()
    }
    ...
    pub(crate) fn update_strategy(&self) -> UpdateStrategy {
        self.spec.update_strategy
    }

    pub(crate) fn update_condition(&self) -> UpdateCondition {
        self.spec.update_condition
    }

    pub(crate) fn shutdown_timeout(&self) -> Option<ShutdownTimeout> {
        self.spec.shutdown_timeout
    }

    pub(crate) fn spec(&self) -> ServiceSpec {
        self.spec.clone()
    }
    ...
    // the bio user account on windows if it existed and no other
    ...
    fn start_health_checks(&mut self) {
        ...
        let mut rx = health::check_repeatedly(
        ...
        let f = async move {
            while let Some(HealthCheckBundle {
                status,
                result,
                interval,
            }) = rx.recv().await
    ...
    fn execute_hooks(
        ...
        match initialization_state {
            ...
            InitializationState::Initialized => {
                let restart_cooldown_period_expired = run_state
                    .restart_backoff
                    .duration_elapsed_since_last_attempt_ended()
                    .map(|duration| -> bool { duration > run_state.restart_config.cooldown_period })
                    .unwrap_or(false);
    ...
    fn update_service_files(&mut self, census_ring: &CensusRing) -> bool {
        let census_group = census_ring.census_group_for(&self.service_group).expect(
            "Service update service files failed; unable to find own service \
                                group",
        );
...
impl TryFrom<SystemTime> for UnixTimestamp {
    ...
    fn try_from(timestamp: SystemTime) -> result::Result<Self, Self::Error> {
        Ok(UnixTimestamp(
            timestamp.duration_since(SystemTime::UNIX_EPOCH)?.as_secs(),
        ))
    }
}
...
impl ServiceQueryModel {
    pub fn new(service: &Service, service_run_state: &ServiceRunState, config_rendering: ConfigRendering) -> Self {
        ServiceQueryModel {
            ...
            health_check: (*service.health_check_result.lock().expect(
                "Couldn't lock health check result for \
                                                  serialization",
            )),
            ...
            process: SupervisedProcessQueryModel::new(
                service
                    .supervisor
                    .lock()
                    .expect(
            ...
            next_restart_at: service_run_state
                .restart_backoff
                .duration_until_next_attempt_start()
                .and_then(|duration| SystemTime::now().checked_add(duration))
                .and_then(|timestamp| UnixTimestamp::try_from(timestamp).ok()),
...
impl From<&ServiceQueryModel> for biome_sup_protocol::types::ServiceStatus {
    fn from(service: &ServiceQueryModel) -> Self {
        Self {
            ident: (*service.pkg.ident.as_ref()).clone().into(),
...
mod tests {
    ...
    use biome_common::types::{GossipListenAddr, HttpListenAddr, ListenCtlAddr};
    use std::{
        net::{IpAddr, Ipv4Addr},
        str::FromStr,
    };
    ...
    async fn initialize_test_service() -> PersistentServiceWrapper {
        ...
        let install = PackageInstall::load(&spec.ident, Some(&path)).expect(
            "PackageInstall \
                                                                             should've loaded my \
                                                                             spec, but it didn't",
        );
        ...
        let gs = Arc::default();
        PersistentServiceWrapper::new(
            Service::with_package(
                asys,
                &install,
                spec,
                afs,
            ...
            .await
            .expect(
                "I wanted a service to load, but it \
                                                            didn't",
            ),
            &ServiceRestartConfig::default(),
        )
    }
    ...
    async fn service_proxy_conforms_to_the_schema() {
        ...
        // With config
        let proxy_with_config = ServiceQueryModel::new(
            service_wrapper.service().unwrap(),
            service_wrapper.service_run_state(),
            ConfigRendering::Full,
        );
        let proxies_with_config = vec![proxy_with_config];
        let json_with_config = serde_json::to_string(&proxies_with_config).expect(
            "Expected to convert \
        ...
        // Without config
        let proxy_without_config = ServiceQueryModel::new(
            service_wrapper.service().unwrap(),
            service_wrapper.service_run_state(),
            ConfigRendering::Redacted,
        );
        let proxies_without_config = vec![proxy_without_config];
        let json_without_config = serde_json::to_string(&proxies_without_config).expect(
            "Expected  to convert \
