...
use crate::{
    census::{CensusGroup, CensusMemberProxy, CensusRing, ElectionStatus},
    manager::Sys,
};
...
use std::{borrow::Cow, collections::BTreeMap, net::IpAddr, path::Path, result};
...
impl<'a> SystemInfo<'a> {
    fn from_sys(sys: &'a Sys) -> Self {
        SystemInfo {
            version: Cow::Borrowed(sys.version.as_str()),
...
impl<'a> Svc<'a> {
    ...
    fn new(census_group: &'a CensusGroup) -> Self {
        Svc {
            ...
            members: census_group.active_members().map(SvcMember::new).collect(),
            me: census_group.me().map(SvcMember::new).expect("Missing 'me'"),
            leader: census_group.leader().map(SvcMember::new),
            update_leader: census_group.update_leader().map(SvcMember::new),
            first: select_first(census_group).expect(
                "First should always be present \
                                                                on svc", /* i.e. `me` will
                                                                          * always be
                                                                          * here, and alive */
            ),
...
mod tests {
    use super::*;
    use crate::{census::CensusMember, manager::service::Cfg, test_helpers::*};
    use biome_butterfly::rumor::service::SysInfo;
    use biome_common::templating::{TemplateRenderer, config::PackageConfigPaths};
...
"#,
    ...
    fn default_svc_member<'a>() -> SvcMember<'a> {
        ...
        let census_member = CensusMember {
    ...
    fn default_render_context<'a>() -> RenderContext<'a> {
        let system_info = SystemInfo {
            version: Cow::Owned("I AM A BIOME VERSION".into()),
    ...
    fn leader_renders_correctly() {
        ...
        let output = render(
            "{{#if bind.foo.leader}}{{bind.foo.leader.member_id}}{{else}}NO \
                             LEADER{{/if}}",
            &ctx,
        );
