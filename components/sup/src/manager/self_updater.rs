...
use tokio::{
    self,
    sync::oneshot::{self, Receiver, Sender, error::TryRecvError},
    time as tokiotime,
};
...
biome_core::env_config_duration!(
    ...
    SelfUpdatePeriod,
    BIO_SUP_UPDATE_MS => from_millis,
    DEFAULT_PERIOD);

impl SelfUpdatePeriod {
    fn get() -> Option<Duration> {
        #[allow(clippy::question_mark)]
        if biome_core::env::var(SelfUpdatePeriod::ENVVAR).is_err() {
            return None;
        }
...
impl SelfUpdater {
    ...
    async fn run(tx: Sender<PackageInstall>, runner: Runner) {
        ...
        debug!(
            "Starting self updater with current package {} in {}s",
            current,
            splay.as_secs()
        );
