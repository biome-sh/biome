...
use crate::{
    ...
    manager::{
        ManagerState,
        action::{ActionSender, SupervisorAction},
        service::{DesiredState, spec::ServiceSpec},
    },
    util,
};
use biome_butterfly as butterfly;
use biome_common::{command::package::install::InstallSource, outputln, ui::UIWriter};
use biome_core::{
...
use std::{convert::TryFrom, str, sync::atomic::Ordering};

static LOGKEY: &str = "CMD";

/// # Locking (see locking.md)
/// * `ManagerServices::inner` (read)
pub fn service_cfg_msr(
    mgr: &ManagerState,
    req: &mut CtlRequest,
...
pub fn service_cfg_validate(
    ...
    let cfg_str = str::from_utf8(&cfg).map_err(|e| {
        net::err(
            ErrCode::BadPayload,
...
pub fn service_unload(
    ...
    if let Some(service_spec) = mgr.cfg.spec_for_ident(&ident) {
        ...
        let action = SupervisorAction::UnloadService {
            service_spec,
            shutdown_input,
        };
        send_action(action, action_sender)?;
    ...
    match mgr.cfg.spec_for_ident(&ident) {
        Some(service_spec) => {
            if service_spec.desired_state == DesiredState::Up {
                let shutdown_input = opts.into();
                let action = SupervisorAction::StopService {
                    service_spec,
                    shutdown_input,
                };
                send_action(action, action_sender)?;
...
pub fn service_status_gsr(mgr: &ManagerState, req: &mut CtlRequest, opts: protocol::ctl::SvcStatus) -> NetResult<()> {
    if let Some(ident) = opts.ident {
        ...
        let service_statuses: Vec<_> = mgr
            .gateway_state
            .lock_gsr()
            .services_data()
            .iter()
