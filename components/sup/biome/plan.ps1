...
function Invoke-Prepare {
    ...
    $env:LIBZMQ_PREFIX              = "$(Get-BioPackagePath "zeromq")"
...
function Invoke-Install {
    Copy-Item -Path "$env:CARGO_TARGET_DIR/release/bio-sup.exe" -Destination "$pkg_prefix/bin/bio-sup.exe"
    Copy-Item -Path "$PLAN_CONTEXT/../static/named_pipe_service.ps1" -Destination "$pkg_prefix/bin/named_pipe_service.ps1"
    Copy-Item -Path "$(Get-BioPackagePath "zeromq")/bin/*.dll" -Destination "$pkg_prefix/bin"
    Copy-Item -Path "$(Get-BioPackagePath "visual-cpp-redist-2022")/bin/*.dll" -Destination "$pkg_prefix/bin"
