use biome_core::{
    self as bio_core, util,
    util::text_render::{TabularText, tabify, tabw},
};
...
pub use crate::{
    builder::{API_RETRY_COUNT, API_RETRY_DELAY, BuilderAPIClient},
    error::{APIFailure, Error, Result},
};
    ...
    fn as_tabbed(&self) -> std::result::Result<String, biome_core::error::Error> {
        ...
        if !self.0.is_empty() {
            let mut body = vec![String::from(
                "Invitation Id\tOrigin Name\tAccount \
...
impl TabularText for OriginInfoResponse {
    fn as_tabbed(&self) -> std::result::Result<String, biome_core::error::Error> {
        ...
        body.push(format!(
            "{}\t{}\t{}\t{}",
            self.owner_id, self.owner_account, key_display, self.default_package_visibility
        ));
        tabify(tw, &body.join("\n"))
    }
}
...
impl FromStr for OriginKeyIdent {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        Ok(OriginKeyIdent {
            origin: caps.get(1).unwrap().as_str().to_string(),
