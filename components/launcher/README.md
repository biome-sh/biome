...
```
token=MY_BEARER_TOKEN
curl -H "Content-type: application/json" -H "Authorization: Bearer ${token}"  https://ci.appveyor.com/api/builds -X POST --data "{'accountName': 'biome','projectSlug': 'windows-service','branch': 'master'}"
```
...
To release a new version of the launcher, upload the new `.bart` file and then [promote it to stable in builder](https://bldr.biome.sh/#/pkgs/biome/bio-launcher). If a new release contains important features or bug fixes, it will have to be communicated to the community and installed manually. Because of the nature of the launcher, this will require downtime in production environments, but it should be an exceedingly rare occurrence.
