...
fn two_members_share_service_files() {
    ...
    net.mesh_mlw_smr();
    net.add_service_file(0, "witcher", "yeppers", "I like to have contents in my file");
...
fn service_file_via_client() {
    ...
    net.wait_for_gossip_rounds(1);
    let mut client = Client::new(&net[0].gossip_addr().to_string(), None).expect(
        "Cannot create Butterfly \
                                                                     Client",
    );
