...
pub use self::{
    ...
    storage::{RumorStore, RumorStoreProxy},
};
...
lazy_static! {
    static ref IGNORED_RUMOR_COUNT: IntCounterVec = register_int_counter_vec!(
        "bio_butterfly_ignored_rumor_total",
...
mod storage {
    use super::*;
    use biome_common::sync::{Lock, ReadGuard};
    use serde::{
    ...
    impl<R> ServiceGroupRumors<'_, R> {
        ...
        /// ```
        /// # use biome_butterfly::rumor::{RumorStore,
        /// #                                Service};
    ...
    impl<'a, T> IterableGuard<'a, RumorMap<T>> {
        fn read(lock: &'a Lock<RumorMap<T>>) -> Self {
            IterableGuard(lock.read())
        }
        ...
        pub fn rumors(&self) -> impl Iterator<Item = &T> {
            self.values().flat_map(HashMap::values)
        }
    ...
    impl<T> RumorStore<T> {
        ...
        pub fn lock_rsr(&self) -> IterableGuard<'_, RumorMap<T>> {
            IterableGuard::read(&self.list)
        }
    ...
    mod tests {
        ...
        fn update_counter_overflows_safely() {
            let rs = RumorStore::<()> {
                update_counter: Arc::new(AtomicUsize::new(usize::MAX)),
                ..Default::default()
            };
