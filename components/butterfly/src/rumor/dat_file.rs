...
impl DatFileReader {
    ...
    pub fn read_or_create_rsr_mlr(
        ...
        if size == 0 {
            DatFileWriter::new(data_path.clone()).write_rsr_mlr(
                member_list,
                service_store,
    ...
    pub fn read(data_path: PathBuf) -> Result<Self> {
        Self::reader_creation(data_path)
    }
    ...
    fn reader_creation(data_path: PathBuf) -> Result<Self> {
        ...
        let dat_file_reader = DatFileReader {
            header,
...
impl DatFileWriter {
    pub fn new(data_path: PathBuf) -> Self {
        DatFileWriter(DatFile(data_path))
    }
    ...
    pub fn write_rsr_mlr(
        ...
        w.with_writer(|mut f| {
            ...
            writer
                .write_all(&[CURRENT_HEADER_VERSION])
                .map_err(|err| Error::DatFileIO(self.path().to_path_buf(), err))?;
            writer
                .write_all(&header_reserve)
                .map_err(|err| Error::DatFileIO(self.path().to_path_buf(), err))?;
            header.insert_member_offset(self.write_member_list_mlr(&mut writer, member_list)?);
            header.insert_offset_for_rumor(
                Service::MESSAGE_ID,
                self.write_rumor_store_rsr(&mut writer, service_store)?,
            );
            header.insert_offset_for_rumor(
                ServiceConfig::MESSAGE_ID,
                self.write_rumor_store_rsr(&mut writer, service_config_store)?,
            );
            header.insert_offset_for_rumor(
                ServiceFile::MESSAGE_ID,
                self.write_rumor_store_rsr(&mut writer, service_file_store)?,
            );
            header.insert_offset_for_rumor(
                Election::MESSAGE_ID,
                self.write_rumor_store_rsr(&mut writer, election_store)?,
            );
            header.insert_offset_for_rumor(
            ...
            header.insert_offset_for_rumor(
                ...
                self.write_rumor_store_rsr(&mut writer, departure_store)?,
            );
...
impl DatFile {
    fn read_header(path: &Path, reader: &mut BufReader<File>) -> Result<Header> {
        ...
        if version[0] < OLDEST_HEADER_VERSION || version[0] > CURRENT_HEADER_VERSION {
            let msg = format!(
                "Unable to read Dat File {}: invalid header version: {}",
                path.display(),
                version[0]
            );
        ...
        reader
            .seek(SeekFrom::Start(header.header_offset()))
            .map_err(|err| Error::DatFileIO(path.to_path_buf(), err))?;
        Ok(header)
    }
...
impl Header {
    ...
    fn member_offset(&self) -> Option<u64> {
        self.offsets.get(Membership::MESSAGE_ID).copied()
    }
...
mod tests {
    ...
    fn read_or_create_mlr_successfully_creates_when_no_file_exists() {
        ...
        let result = DatFileReader::read_or_create_rsr_mlr(
            file_path.clone(),
            &MemberList::new(),
            &RumorStore::default(),
            &RumorStore::default(),
