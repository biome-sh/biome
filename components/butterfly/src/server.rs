...
use self::{incarnation_store::IncarnationStore, sync::Myself};
use crate::{
    error::{Error, Result},
    ...
    rumor::{
        ConstIdRumor, Rumor, RumorKey, RumorStore, RumorStoreProxy, RumorType,
        dat_file::{DatFileReader, DatFileWriter},
        departure::Departure,
...
use biome_common::{FeatureFlag, liveliness_checker, sync::Lock};
use biome_core::crypto::keys::RingKey;
use lazy_static::lazy_static;
use log::{debug, error, info, trace, warn};
use prometheus::{HistogramTimer, HistogramVec, IntGauge, opts, register_histogram_vec, register_int_gauge};
...
pub(crate) mod sync {
    ...
    impl<'a> MyselfReadGuard<'a> {
        fn new(lock: &'a Lock<MyselfInner>) -> Self {
            Self(lock.read())
        }
    ...
    impl<'a> MyselfWriteGuard<'a> {
        ...
        pub fn sync_incarnation(&mut self, store: incarnation_store::IncarnationStore) -> Result<()> {
            self.0.sync_incarnation(store)
        }

        pub fn increment_incarnation(&mut self) {
            self.0.increment_incarnation()
        }
        ...
        pub fn mark_departed(&mut self) {
            self.0.mark_departed()
        }

        pub fn set_persistent(&mut self) {
            self.0.set_persistent()
        }
    }
    ...
    impl Myself {
        ...
        #[must_use]
        pub fn lock_smr(&self) -> MyselfReadGuard<'_> {
            MyselfReadGuard::new(&self.inner)
        }

        #[must_use]
        pub fn lock_smw(&self) -> MyselfWriteGuard<'_> {
            MyselfWriteGuard::new(&self.inner)
        }
    }
...
impl Server {
    ...
    pub fn start_rsw_mlw_smw_rhw_msr(&mut self, timing: &timing::Timing) -> Result<()> {
        ...
        if let Some(ref path) = self.data_path {
            ...
            let mut reader = DatFileReader::read_or_create_rsr_mlr(
                dat_path.clone(),
                &self.member_list,
                &self.service_store,
            ...
            match reader.read_into_rsw_mlw_rhw_msr(self) {
                Ok(_) => {
                    debug!("Successfully ingested rumors from {}", reader.path().display())
                }
        ...
        inbound::spawn_thread(
            format!("inbound-{}", self.name()),
            self.clone(),
            clone_socket(&socket)?,
            tx_outbound,
        )?;

        outbound::spawn_thread(
            format!("outbound-{}", self.name()),
            self.clone(),
            socket,
            rx_inbound,
            timing.clone(),
        ...
        expire::spawn_thread(format!("expire-{}", self.name()), self.clone(), timing.clone())?;

        pull::spawn_thread(format!("pull-{}", self.name()), self.clone())?;

        push::spawn_thread(format!("push-{}", self.name()), self.clone(), timing.clone())?;
    ...
    pub fn need_peer_seeding_mlr(&self) -> bool {
        self.member_list.is_empty_mlr()
    }
    ...
    fn insert_member_from_rumor_mlw_smw_rhw(&self, member: Member, mut health: Health) {
        ...
        if member.id != self.member_id() {
                    ...
                    match mship.health {
                        Health::Alive | Health::Suspect => {
                            if mship.member.incarnation > member.incarnation {
                                debug!(
                                    "Member: {}, Incoming Incarnation {} is older, current \
                                        incarnation is {}. No-OP.",
                                    member.id, member.incarnation, mship.member.incarnation
                                );
                            } else {
                                warn!(
                                    "Member: {}, Our Information about the member is '{}', \
                        ...
                        _ => {
                            debug!(
                                "Member: {}, Incoming Health {}, Member Health: {}. No-op",
            ...
            trace!(
                "member incarnation is {}. my incarnation is {}",
                member.incarnation,
                self.myself.lock_smr().incarnation()
            );
            health = Health::Alive;
            if member.incarnation >= self.myself.lock_smr().incarnation() {
                self.myself.lock_smw().refute_incarnation(member.incarnation);
            }
        }
    ...
    pub fn insert_service_rsw_mlw_rhw(&self, service: Service) {
        Self::insert_service_impl(service, &self.service_store, &self.member_list, &self.rumor_heat, |k| {
    ...
    fn insert_service_impl(
        ...
        let inserting_new_group_member = service_store
            .lock_rsr()
            .get(service_group)
        ...
        if service_store.insert_rsw(service) {
            if inserting_new_group_member && !check_quorum(service_group) {
                ...
                if let Some(member_id_to_depart) = service_store
                    ...
                    .rumors()
                    .map(Rumor::id)
                    .filter(|id| member_list.health_of_by_id_mlr(id) == Some(Health::Confirmed))
                    .min()
                {
                    ...
                    rumor_heat
                        .lock_rhw()
                        .start_hot_rumor(RumorKey::new(RumorType::Member, member_id_to_depart, ""));
                }
            }
    ...
    fn check_quorum_mlr(&self, key: &str) -> bool {
        ...
        let quorum_log_entry = format!(
            "check_quorum({}): {}/{} alive/total => {}, electorate: \
                                        {:?}, service_group: {:?}",
            key, alive_population, total_population, has_quorum, electorate, service_group_members
        );
        if !has_quorum {
    ...
    /// * `RumorHeat::inner` (write)
    /// * `ManagerServices::inner` (read)
    pub fn start_election_rsw_mlr_rhw_msr(&self, service_group: &str, term: u64, suitability: Option<u64>) {
        let suitability = suitability.unwrap_or_else(|| self.suitability_lookup.suitability_for_msr(service_group));
        let has_quorum = self.check_quorum_mlr(service_group);
        let e = Election::new(self.member_id(), service_group, term, suitability, has_quorum);
        if !has_quorum {
            warn!("start_election check_quorum failed: {:?}", e);
        }
        debug!("start_election: {:?}", e);
        self.rumor_heat.lock_rhw().start_hot_rumor(RumorKey::from(&e));
    ...
    /// * `MemberList::entries` (read)
    /// * `RumorHeat::inner` (write)
    pub fn start_update_election_rsw_mlr_rhw(&self, service_group: &str, suitability: u64, term: u64) {
        let has_quorum = self.check_quorum_mlr(service_group);
        let e = ElectionUpdate::new(self.member_id(), service_group, term, suitability, has_quorum);
        if !has_quorum {
            warn!("start_election check_quorum failed: {:?}", e);
        }
    ...
    /// * `RumorStore::list` (read)
    /// * `MemberList::entries` (read)
    fn elections_to_restart_rsr_mlr<T>(
        &self,
    ...
    where
        T: ConstIdRumor + ElectionRumor + Debug,
    {
        ...
        for (service_group, rumors) in elections.lock_rsr().iter() {
                ...
                .contains_id(myself_member_id)
            {
                ...
                debug!("elections_to_restart: checking {} -> {:#?}", service_group, election);
                ...
                if election_trigger::maybe_trigger(service_group, feature_flags, data_path) {
                    ...
                    if election.is_finished() && election.member_id() == myself_member_id {
                        ...
                        let leader_health =
                            member_list
                                .health_of_by_id_mlr(election.member_id())
                                .unwrap_or_else(|| {
                                    debug!(
                                        "No health information for {}; \
    ...
    pub fn restart_elections_rsw_mlr_rhw_msr(&self, feature_flags: FeatureFlag) {
        let elections_to_restart = self.elections_to_restart_rsr_mlr(&self.election_store, feature_flags);
        ...
        let update_elections_to_restart = self.elections_to_restart_rsr_mlr(&self.update_store, feature_flags);

        for (service_group, old_term) in elections_to_restart {
            let term = old_term + 1;
            warn!("Starting a new election for {} {}", service_group, term);
            let suitability = if Some(self.member_id()) == self.election_store.lock_rsr().get_member_id(&service_group)
            {
                Some(u64::MAX)
            } else {
                None
            ...
            self.election_store.remove_rsw(&service_group, Election::const_id());
        ...
        for (service_group, old_term) in update_elections_to_restart {
            let term = old_term + 1;
            warn!("Starting a new election for {} {}", service_group, term);
            self.update_store.remove_rsw(&service_group, ElectionUpdate::const_id());
    ...
    pub fn insert_election_rsw_mlr_rhw_msr(&self, mut election: Election) {
        ...
        {
            trace!("{} is a member of {}", self.member_id(), election.service_group);
            ...
            if self.election_store.lock_rsr().contains_rumor(&election) {
                ...
                if new_term {
                    if Some(self.member_id()) == self.election_store.lock_rsr().get_member_id(election.key()) {
                        ...
                        debug!(
                            "removing old rumor and starting new election with highest \
                                `Suitability`"
                        );
                        self.election_store.remove_rsw(election.key(), election.id());
                        self.start_election_rsw_mlr_rhw_msr(&election.service_group, election.term, Some(u64::MAX));
                    } else if Some(election.member_id.as_str())
                        == self.election_store.lock_rsr().get_member_id(election.key())
                    {
                        debug!(
                            "Received New Term election from the leader. Starting my own to \
                                merge. Term: {}",
                            election.term
                        );
                        self.election_store.remove_rsw(election.key(), election.id());
                        ...
                        warn!(
                            "Received a New Term Election, but not from the current leader, \
                               ignoring!"
                        );
                        return;
                    }
                ...
                if self.member_id() == election.member_id {
                    if self.check_quorum_mlr(election.key()) {
                        ...
                        if num_votes == electorate.len() {
                            ...
                            let mut existing_timers =
                                self.election_timers.lock().expect("Election timers lock poisoned");
                ...
                let timer = ELECTION_DURATION
                    ...
                    .start_timer();
                let mut existing_timers = self.election_timers.lock().expect("Election timers lock poisoned");
...
impl fmt::Display for Server {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}@{}/{}", self.name(), self.swim_port(), self.gossip_port())
    }
}
...
fn persist_loop(server: &Server) -> ! {
    ...
    loop {
        ...
        match min_loop_period.checked_sub(time_to_persist) {
            ...
            None => {
                warn!("Persisting data took longer than expected: {:?}", time_to_persist)
            }
        }
    }
}
...
fn clone_socket(socket: &UdpSocket) -> Result<UdpSocket> {
    ...
    match unsafe {
        handleapi::SetHandleInformation(cloned.as_raw_socket() as winnt::HANDLE, winbase::HANDLE_FLAG_INHERIT, 0)
    } {
...
#[cfg(unix)]
fn clone_socket(socket: &UdpSocket) -> std::io::Result<UdpSocket> {
    socket.try_clone()
}
...
mod election_trigger {
    ...
    /// If the BIO_FEAT_TRIGGER_ELECTION feature flag is enabled,
    ...
    ///
    /// - the supervisor is running with `BIO_FEAT_TRIGGER_ELECTION=1`
    /// - the file `/bio/sup/default/data/trigger_foo.default_election` is present _and can also be
    ...
    /// that intent.
    pub(super) fn maybe_trigger(
        service_group_name: &str,
        feature_flags: FeatureFlag,
        data_path: &Option<PathBuf>,
    ) -> bool {
        if !feature_flags.contains(FeatureFlag::TRIGGER_ELECTION) {
            trace!("BIO_FEAT_TRIGGER_ELECTION: Feature not enabled");
            return false;
        }

        if data_path.is_none() {
            trace!("BIO_FEAT_TRIGGER_ELECTION: In a test");
            return false;
        }

        let sentinel_file = data_path
            .as_ref()
            .unwrap() // safe; data_path is Some
            .join(format!("trigger_{}_election", service_group_name));
        if !sentinel_file.is_file() {
            trace!(
                "BIO_FEAT_TRIGGER_ELECTION: No trigger file found for {}",
                service_group_name
            );
            return false;
        }
        ...
        if let Err(e) = fs::remove_file(&sentinel_file) {
            ...
            // trigger elections.
            warn!(
                "BIO_FEAT_TRIGGER_ELECTION: Could not delete sentinel file {:?}; NOT \
                   triggering election! {:?}",
                sentinel_file, e
            );
            false
            ...
            info!(
                "BIO_FEAT_TRIGGER_ELECTION: Manually triggering election for {}",
                service_group_name
            );
            true
        }
...
mod tests {
    ...
    fn insert_service_with_new_member_departs_confirmed_member() {
        ...
        Server::insert_service_impl(
            confirmed_member_service_rumor,
            &service_store,
            &member_list,
            &rumor_heat,
            check_quorum_returns(false),
        ...
        assert_eq!(member_list.health_of_mlr(&confirmed_member), Some(Health::Confirmed));
        ...
        assert_eq!(member_list.health_of_mlr(&confirmed_member), Some(Health::Departed));
    }
    ...
    mod myself {
            ...
            P: AsRef<Path>,
        {
            ...
            i.initialize().expect("Couldn't initialize incarnation store");
