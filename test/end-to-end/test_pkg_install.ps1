...
Describe "pkg install" {
    BeforeEach {
        ...
        bio pkg uninstall $env:BIO_ORIGIN/dep-pkg-3
    ...
    It "installs all dependencies and executes all install hooks" {
        ...
        Get-Content "$(bio pkg path $env:BIO_ORIGIN/dep-pkg-2)/INSTALL_HOOK_STATUS" | Should -Be "0"
        Get-Content "$(bio pkg path $env:BIO_ORIGIN/dep-pkg-3)/INSTALL_HOOK_STATUS" | Should -Be "0"
    }
    ...
    It "installs all dependencies and executes all install hooks in the correct order" {
        $cached = Get-Item "/bio/cache/artifacts/$env:BIO_ORIGIN-dep-pkg-4*"
        bio pkg install $cached.FullName
        $LASTEXITCODE | Should -Be 0
        Get-Content "$(bio pkg path $env:BIO_ORIGIN/dep-pkg-1)/INSTALL_HOOK_STATUS" | Should -Be "0"
        Get-Content "$(bio pkg path $env:BIO_ORIGIN/dep-pkg-2)/INSTALL_HOOK_STATUS" | Should -Be "0"
        Get-Content "$(bio pkg path $env:BIO_ORIGIN/dep-pkg-4)/INSTALL_HOOK_STATUS" | Should -Be "0"
    }

    It "installs any missing transitive dependency that may have been removed" {
        $dep3_artifact = Get-Item "/bio/cache/artifacts/$env:BIO_ORIGIN-dep-pkg-3*"

        bio pkg install $dep3_artifact
        $LASTEXITCODE | Should -Be 0

        $dep1_install = "$(bio pkg path $env:BIO_ORIGIN/dep-pkg-1)"
        $dep1_install_hook_status = "$dep1_install/INSTALL_HOOK_STATUS"

        $dep2_install = "$(bio pkg path $env:BIO_ORIGIN/dep-pkg-2)"
