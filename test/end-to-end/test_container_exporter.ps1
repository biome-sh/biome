...
function New-Image() {
    ...
    Write-Host (bio pkg export container --base-pkgs-channel=$env:HAB_BLDR_CHANNEL core/nginx --tag-custom=$tag $extra_args | Out-String)
    "core/nginx:$tag"
}
...
# This is just to ensure that our deprecated `bio pkg export docker`
# alias still functions, until we decide to remove it.
Describe "Old 'bio pkg export docker' alias" {
    BeforeAll {
        $tag = New-CustomTag
        Write-Host (bio pkg export docker --base-pkgs-channel=$env:HAB_BLDR_CHANNEL core/nginx --tag-custom=$tag | Out-String)
...
if ($IsLinux) {
    ...
    Describe "bio pkg export container --engine=buildah" {
        It "Runs successfully" {
            $tag = New-CustomTag
            Invoke-NativeCommand bio pkg export container --base-pkgs-channel=$env:HAB_BLDR_CHANNEL core/nginx --engine=buildah --tag-custom="$tag"
