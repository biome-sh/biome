Describe "Finding peers from watch file" {
    It "finds all three peers" {
        $testScript = { (Invoke-WebRequest "http://bastion.biome.dev:9631/census" | ConvertFrom-Json).last_membership_counter -eq 3 }
        ...
        $json = (Invoke-WebRequest "http://bastion.biome.dev:9631/census" | ConvertFrom-Json)
