...
$env:BIO_AUTH_TOKEN = $env:BIO_AUTH_TOKEN.Replace("`"", "")
...
Describe "bio svc update" {
    ...
    Context "with a bound service" {
        ...
        It "starts test-probe bound to nginx" {
            '/bio/sup/default/specs/test-probe.spec' | Should -FileContentMatchExactly 'binds = \["thing_with_a_port:nginx.default"\]'
        }
    ...
    Context "removing binds via bio svc update" {
        ...
        It "has the new binds in the spec file" {
            '/bio/sup/default/specs/test-probe.spec' | Should -FileContentMatchExactly 'binds = \[\]'
        }
    ...
    Context "updating another field does not wipe out existing binds" {
        ...
        It "updates the update strategy" {
            '/bio/sup/default/specs/test-probe.spec' | Should -FileContentMatchExactly 'update_strategy = "at-once"'
        }
    ...
    Context "an updating service" {
        ...
        It "reflects channel in spec file" {
            '/bio/sup/default/specs/nginx.spec' | Should -FileContentMatchExactly "channel = `"$testChannelOne`""
        }
        ...
        It "has the new channel in the spec file" {
            '/bio/sup/default/specs/nginx.spec' | Should -FileContentMatchExactly "channel = `"$testChannelTwo`""
            '/bio/sup/default/specs/nginx.spec' | Should -FileContentMatchExactly 'update_strategy = "at-once"'
        }
