...
# by bio. We don't have a way to inspect loaded certificates at this
...
Describe "Self signed cert"  {
    if ($IsMacOS) {
        New-Item -ItemType Directory /opt/bio/cache/ssl
        $e2e_cert = "/opt/bio/cache/ssl/e2e-ssl.pem"
    } else {
        New-Item -ItemType Directory /bio/cache/ssl
        $e2e_cert = "/bio/cache/ssl/e2e-ssl.pem"
    }
    ...
    It "should be used by bldr client" {
        $env:RUST_LOG="debug"
        Start-Process bio -ArgumentList "pkg search core/redis" -RedirectStandardError err.log -Wait
